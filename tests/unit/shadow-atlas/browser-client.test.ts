...
describe('browser-client', () => {
	...
	describe('computeDistrictCommitment', () => {
		it('produces consistent SHA-256 output for same input (fallback path)', async () => {
			// Mock poseidon to throw so we fall through to SHA-256
			...
			const { computeDistrictCommitment } = await import(
				'$lib/core/shadow-atlas/browser-client'
			);

			const slots = {
				slots: ['cd-0612', 'CA', null, null, ...Array(20).fill(null)],
			};

			const result1 = await computeDistrictCommitment(slots);
			const result2 = await computeDistrictCommitment(slots);

			expect(result1.commitment).toBe(result2.commitment);
			expect(result1.commitment).toHaveLength(64); // SHA-256 hex
			expect(result1.slotCount).toBe(2);
		});

		it('counts non-empty slots correctly', async () => {
			vi.doMock('$lib/core/crypto/poseidon', () => {
				throw new Error('WASM not available');
			});
			vi.doMock('$lib/core/shadow-atlas/ipfs-store', () => ({
			...
			const allNull = { slots: Array(24).fill(null) };
			const result = await computeDistrictCommitment(allNull);
		...
		it('produces different commitments for different inputs', async () => {
			...
			const slotsA = { slots: ['CA-12', ...Array(23).fill(null)] };
			const slotsB = { slots: ['NY-14', ...Array(23).fill(null)] };

			const a = await computeDistrictCommitment(slotsA);
			const b = await computeDistrictCommitment(slotsB);

			expect(a.commitment).not.toBe(b.commitment);
		});

		it('handles slots with fewer than 24 elements by padding', async () => {
			vi.doMock('$lib/core/crypto/poseidon', () => {
				throw new Error('WASM not available');
			});
			vi.doMock('$lib/core/shadow-atlas/ipfs-store', () => ({
			...
			// Only 3 slots provided — should not throw
			const shortSlots = { slots: ['CA-12', null, 'CA'] };
			const result = await computeDistrictCommitment(shortSlots);
