...
export const GET: RequestHandler = async ({ request, params }) => {
	...
	const campaign = await serverInternalQuery(internal.v1api.getCampaignById, { campaignId: params.id, orgId: auth.orgId });
...
export const PATCH: RequestHandler = async ({ request, params }) => {
	...
	const result = await serverInternalMutation(internal.v1api.updateCampaign, { campaignId: params.id, orgId: auth.orgId, data });
