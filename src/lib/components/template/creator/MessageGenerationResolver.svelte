<script lang="ts">
	...
	function buildGenerationPayload(): MessageGenerationPayload {
		...
		return buildMessageGenerationPayload({
			subjectLine,
			coreMessage,
			topics: buildTopics(formData.objective),
			decisionMakers,
			voiceSample: buildVoiceSample(formData.objective),
			rawInput: buildRawInput(formData.objective),
			geographicScope: inferGeoScope({ decisionMakers }).scope
		});
	}
