...
<script lang="ts">
	 ...
	 *
	 * Stage 2.5: `recoverThreeTree` recomputes `districtCommitment` (Poseidon2
	 * sponge-24 over the 24-slot districts array) from the IPFS cell payload
	 * and persists it on the restored SessionCredential. Nothing to thread
...
<div class="mx-auto max-w-3xl">
	{#if mdlGated}
		<!--
			mDL launch gate. Recovery requires government-ID re-verification.
			Document-register placeholder — eyebrow + Satoshi heading + dotted
			rules — instead of dispatching into a flow that 404s.
		-->
		<section class="py-2" data-testid="recovery-mdl-gated">
			...
			<div class="mb-8">
				...
				<div class="mt-2 h-2 w-full rounded-full bg-slate-200">
					<div
						...
						style="width: {currentStep === 'explain'
							? '33%'
			...
			{#if currentStep === 'explain'}
				...
				<div class="py-8 text-center">
					...
					<p class="mx-auto mb-6 max-w-md text-base text-slate-600">
						Your proof credentials were cleared from this device. One quick verification to restore
						them.
					</p>
