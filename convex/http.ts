		...
		try {
				...
				if (metadata?.type === 'donation' && metadata.donationId) {
					await ctx.runMutation(internal.webhooks.completeDonation, {
						...
						stripePaymentIntentId:
							typeof eventData.payment_intent === 'string' ? eventData.payment_intent : undefined,
			...
			if (event.type === 'charge.refunded') {
				const paymentIntentId =
					typeof eventData.payment_intent === 'string' ? eventData.payment_intent : null;
				if (paymentIntentId) {
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		if (body.TopicArn !== allowedTopic) {
			...
			return new Response(JSON.stringify({ ok: false, error: 'topic not allowed' }), {
				status: 403,
				headers: { 'Content-Type': 'application/json' }
			});
		}
		...
		for (const field of requiredFields) {
			if (typeof body[field] !== 'string') {
				return new Response(JSON.stringify({ ok: false, error: `missing or invalid ${field}` }), {
					status: 400,
					headers: { 'Content-Type': 'application/json' }
				});
			}
		...
		if (Math.abs(Date.now() - snsTimestampMs) > SNS_REPLAY_WINDOW_MS) {
			...
			return new Response(JSON.stringify({ ok: false, error: 'Timestamp outside replay window' }), {
				status: 400,
				headers: { 'Content-Type': 'application/json' }
			});
		}
		...
		if (body.Type === 'Notification' && typeof body.Message === 'string') {
			try {
				...
				return new Response(JSON.stringify({ ok: false, error: 'invalid Message JSON' }), {
					status: 400,
					headers: { 'Content-Type': 'application/json' }
				});
			}
		...
		try {
			...
			// SNS retries non-2xx deliveries. Never acknowledge a bounce/complaint
			// until its compact authority + idempotent fanout job committed.
			return new Response(JSON.stringify({ ok: false, error: 'processing unavailable' }), {
				status: 503,
				headers: { 'Content-Type': 'application/json' }
			});
		}
...
async function validateTwilioSignature(
	...
	const mac = await crypto.subtle.sign('HMAC', keyData, new TextEncoder().encode(data));
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		const valid = await validateTwilioSignature(signature, request.url, params);
		if (!valid) {
		...
		try {
			...
			const normalizedStatus = (
				ALLOWED.includes(messageStatus as (typeof ALLOWED)[number]) ? messageStatus : 'queued'
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		if (!from) {
			return new Response(JSON.stringify({ error: 'Missing From number' }), {
				status: 400,
				headers: { 'Content-Type': 'application/json' }
			});
		}
		...
		try {
			...
			// Twilio retries 5xx responses. A success response is reserved for a
			// transaction that durably committed the idempotent ingress job (and,
			// for STOP/START, the synchronous authority row).
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		if (!activeSecret) {
			...
			return new Response(JSON.stringify({ ok: false, error: 'endpoint not configured' }), {
				status: 503,
				headers: { 'Content-Type': 'application/json' }
			});
		}
		if (activeSecret.length < 32) {
			...
			return new Response(JSON.stringify({ ok: false, error: 'endpoint misconfigured' }), {
				status: 503,
				headers: { 'Content-Type': 'application/json' }
			});
		}
		...
		if (!body.blastId || !Array.isArray(body.receipts)) {
			return new Response(JSON.stringify({ ok: false, error: 'blastId and receipts[] required' }), {
				status: 400,
				headers: { 'Content-Type': 'application/json' }
			});
		}
