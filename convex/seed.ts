...
import { syncSupporterBrowseProjection } from './lib/supporterBrowse';
import {
...
// Seed phases operate on a deliberately tiny fixture, but the internal
// functions are still callable with existing organization ids. Keep every
// reconciliation read bounded so an accidental production invocation cannot
// turn a convenience fixture into a whole-roster database-I/O scan.
const SEED_RECONCILIATION_ROW_LIMIT = 128;

function assertSeedReconciliationBound(table: string, count: number): void {
	if (count > SEED_RECONCILIATION_ROW_LIMIT) {
		throw new Error(`SEED_RECONCILIATION_LIMIT_EXCEEDED:${table}:${count}`);
	}
}
...
const zeroTemplateMetricsRef = makeFunctionReference<'mutation'>(
	'seed:zeroTemplateMetrics'
) as unknown as FunctionReference<
	'mutation',
	'internal',
	{ cursor?: string; scheduleContinuation?: boolean },
	unknown
>;
const backfillScopesRef = makeFunctionReference<'mutation'>(
	'seed:backfillScopes'
) as unknown as FunctionReference<
	'mutation',
	'internal',
	{ offset?: number; scheduleContinuation?: boolean },
	unknown
>;
...
const REPRESENTATIVES = [
	...
	{
		name: 'Rep. David Okonkwo',
		title: 'U.S. Representative',
		email: 'okonkwo@house.gov',
		district: 'CA-12'
	},
	{
		name: 'Rep. Sarah Kim',
	...
	{
		name: 'Sen. Robert Huang',
		title: 'State Senator',
		email: 'huang@senate.ca.gov',
		district: 'SD-11'
	},
	{
		name: 'Asm. Lisa Chen',
		title: 'State Assembly',
		email: 'chen@assembly.ca.gov',
		district: 'AD-17'
	},
	...
	{ name: 'Sup. Matt Dorsey', title: 'Supervisor', email: 'dorsey@sfgov.org', district: 'D6' },
	{
		...
		title: 'U.S. Representative',
		email: 'pelosi@house.gov',
		district: 'CA-11'
	},
	{ name: 'Mayor London Breed', title: 'Mayor', email: 'mayor@sfgov.org', district: 'SF' }
] as const;

type RecipientMetricsMigrationRun = {
	status: string;
	runToken?: string;
	failureCode?: string | null;
};

type SessionAuthorityMigrationRun = {
	status: string;
	runToken?: string;
	failureCode?: string | null;
};

type SupporterBrowseMigrationRun = {
	status: string;
	runToken?: string;
	failureCode?: string | null;
};

type SupporterAudienceMigrationRun = {
	status: string;
	startedAt?: number;
	failureCode?: string | null;
};

type CoalitionMetricsMigrationRun = {
	status: string;
	runToken?: string;
...
type NetworkCharterMigrationRun = {
	status: string;
	runToken?: string;
	failureCode?: string | null;
};

const supporterAudienceMigratePageRef = makeFunctionReference<'mutation'>(
	'supporterAudience:migratePage'
) as unknown as FunctionReference<
	'mutation',
	'internal',
	{ startedAt?: number; restart?: boolean; scheduleContinuation?: boolean },
	SupporterAudienceMigrationRun
>;
const supporterAudienceActivateRef = makeFunctionReference<'mutation'>(
	'supporterAudience:activate'
) as unknown as FunctionReference<
	'mutation',
	'internal',
	Record<string, never>,
	{ status: string; scanned?: number }
>;

/**
 * Seed/reseed operates on a deliberately small corpus, so finish the bounded
 * two-phase recipient-metrics migration synchronously and activate its readers
 * before returning. Production legacy cutover uses the self-paging CLI path.
 */
async function ensureRecipientMetricsReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(internal.templatePage.migrateRecipientMetrics, {
		scheduleContinuation: false
	})) as RecipientMetricsMigrationRun;
	for (let page = 0; page < 256; page += 1) {
		if (result.status === 'ready' || result.status === 'already-ready') return;
		if (result.status === 'migrated' || result.status === 'already-migrated') {
			await ctx.runMutation(internal.templatePage.activateRecipientMetrics, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(
				`RECIPIENT_METRICS_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`
			);
		}
		if (result.status === 'superseded') {
			result = (await ctx.runMutation(internal.templatePage.migrateRecipientMetrics, {
				scheduleContinuation: false
			})) as RecipientMetricsMigrationRun;
			continue;
		}
		if ((result.status !== 'running' && result.status !== 'already-running') || !result.runToken) {
			throw new Error(`RECIPIENT_METRICS_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(internal.templatePage.migrateRecipientMetrics, {
	...
	throw new Error('RECIPIENT_METRICS_SEED_MIGRATION_PAGE_LIMIT_EXCEEDED');
}

/**
 * A seed action is not complete until its compact authentication projection is
 * active. Keep the same bounded migrate-then-activate contract used by the
 * production cutover instead of teaching development data a privileged path.
 */
async function ensureSessionAuthorityReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(internal.sessionAuthority.migrateSessionAuthorities, {
		scheduleContinuation: false
	})) as SessionAuthorityMigrationRun;
	for (let page = 0; page < 256; page += 1) {
		if (result.status === 'ready' || result.status === 'already-ready') return;
		if (result.status === 'migrated' || result.status === 'already-migrated') {
			await ctx.runMutation(internal.sessionAuthority.activateSessionAuthorities, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(
				`SESSION_AUTHORITY_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`
			);
		}
		if (result.status === 'superseded') {
			result = (await ctx.runMutation(internal.sessionAuthority.migrateSessionAuthorities, {
				scheduleContinuation: false
			})) as SessionAuthorityMigrationRun;
			continue;
		}
		if ((result.status !== 'running' && result.status !== 'already-running') || !result.runToken) {
			throw new Error(`SESSION_AUTHORITY_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(internal.sessionAuthority.migrateSessionAuthorities, {
			runToken: result.runToken,
			scheduleContinuation: false
	...
	throw new Error('SESSION_AUTHORITY_SEED_MIGRATION_PAGE_LIMIT_EXCEEDED');
}

/**
 * Seed writes intentionally exercise the same legacy-row migration production
 * uses. Restart closes readers before the scan, then bounded pages rebuild tag
 * markers/counts and inline browse fields before the separate activation.
 */
async function ensureSupporterBrowseReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(internal.supporters.migrateSupporterBrowse, {
		restart: true,
		scheduleContinuation: false
	})) as SupporterBrowseMigrationRun;
	for (let page = 0; page < 4_096; page += 1) {
		if (result.status === 'ready' || result.status === 'already-ready') return;
		if (result.status === 'migrated' || result.status === 'already-migrated') {
			await ctx.runMutation(internal.supporters.activateSupporterBrowse, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(`SUPPORTER_BROWSE_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`);
		}
		if (result.status === 'superseded') {
			result = (await ctx.runMutation(internal.supporters.migrateSupporterBrowse, {
				restart: true,
				scheduleContinuation: false
			})) as SupporterBrowseMigrationRun;
			continue;
		...
		if ((result.status !== 'running' && result.status !== 'already-running') || !result.runToken) {
			throw new Error(`SUPPORTER_BROWSE_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(internal.supporters.migrateSupporterBrowse, {
			runToken: result.runToken,
			scheduleContinuation: false
	...
	throw new Error('SUPPORTER_BROWSE_SEED_MIGRATION_PAGE_LIMIT_EXCEEDED');
}

/** Activate the compact campaign-action dimensions consumed by saved segments. */
async function ensureSupporterAudienceReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(supporterAudienceMigratePageRef, {
		restart: true,
		scheduleContinuation: false
	})) as SupporterAudienceMigrationRun;
	for (let page = 0; page < 4_096; page += 1) {
		if (result.status === 'ready') return;
		if (result.status === 'migrated') {
			await ctx.runMutation(supporterAudienceActivateRef, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(
				`SUPPORTER_AUDIENCE_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`
			);
		}
		if (result.status !== 'running' || result.startedAt === undefined) {
			throw new Error(`SUPPORTER_AUDIENCE_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(supporterAudienceMigratePageRef, {
			startedAt: result.startedAt,
			scheduleContinuation: false
		})) as SupporterAudienceMigrationRun;
	}
	throw new Error('SUPPORTER_AUDIENCE_SEED_MIGRATION_PAGE_LIMIT_EXCEEDED');
}

/** Drive both the raw cutover and every bounded network generation to ready. */
async function ensureCoalitionMetricsReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(internal.networks.migrateCoalitionMetrics, {
		restart: true,
		scheduleContinuation: false
	})) as CoalitionMetricsMigrationRun;
	for (let step = 0; step < 2_048; step += 1) {
		if (result.status === 'ready' || result.status === 'already-ready') return;
		if (result.status === 'migrated' || result.status === 'already-migrated') {
			await ctx.runMutation(internal.networks.activateCoalitionMetrics, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(
				`COALITION_METRICS_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`
			);
		}
		if (result.status === 'waiting') {
			if (!result.networkId || !result.runToken) {
				throw new Error('COALITION_METRICS_SEED_WAIT_STATE_INVALID');
			}
			await ctx.runMutation(internal.networks.continueCoalitionNetworkRebuild, {
				networkId: result.networkId
			});
			result = (await ctx.runMutation(internal.networks.migrateCoalitionMetrics, {
				runToken: result.runToken,
				scheduleContinuation: false
			})) as CoalitionMetricsMigrationRun;
			continue;
		...
		if ((result.status !== 'running' && result.status !== 'already-running') || !result.runToken) {
			throw new Error(`COALITION_METRICS_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(internal.networks.migrateCoalitionMetrics, {
			runToken: result.runToken,
			scheduleContinuation: false
	...
	throw new Error('COALITION_METRICS_SEED_MIGRATION_STEP_LIMIT_EXCEEDED');
}

async function ensureNetworkChartersReadyAfterSeed(ctx: ActionCtx): Promise<void> {
	let result = (await ctx.runMutation(internal.networks.migrateNetworkCharters, {
		restart: true,
		scheduleContinuation: false
	})) as NetworkCharterMigrationRun;
	for (let page = 0; page < 256; page += 1) {
		if (result.status === 'ready' || result.status === 'already-ready') return;
		if (result.status === 'migrated' || result.status === 'already-migrated') {
			await ctx.runMutation(internal.networks.activateNetworkCharters, {});
			return;
		}
		if (result.status === 'blocked') {
			throw new Error(`NETWORK_CHARTER_SEED_MIGRATION_BLOCKED:${result.failureCode ?? 'unknown'}`);
		}
		if (result.status === 'superseded') {
			result = (await ctx.runMutation(internal.networks.migrateNetworkCharters, {
				scheduleContinuation: false
			})) as NetworkCharterMigrationRun;
			continue;
		}
		if ((result.status !== 'running' && result.status !== 'already-running') || !result.runToken) {
			throw new Error(`NETWORK_CHARTER_SEED_MIGRATION_INVALID:${result.status}`);
		}
		result = (await ctx.runMutation(internal.networks.migrateNetworkCharters, {
			runToken: result.runToken,
			scheduleContinuation: false
	...
	throw new Error('NETWORK_CHARTER_SEED_MIGRATION_PAGE_LIMIT_EXCEEDED');
}
...
export const seedAll = internalAction({
	...
	handler: async (ctx) => {
		...
		if (existing) {
			...
			await ensureRecipientMetricsReadyAfterSeed(ctx);
			await ensureSessionAuthorityReadyAfterSeed(ctx);
			await ensureSupporterBrowseReadyAfterSeed(ctx);
			await ensureSupporterAudienceReadyAfterSeed(ctx);
			await ensureCoalitionMetricsReadyAfterSeed(ctx);
			await ensureNetworkChartersReadyAfterSeed(ctx);
			return;
		}
		...
		const campaignIds = await ctx.runMutation(internal.seed.insertCampaigns, {
			orgIds,
			templateIds
		});
		...
		await ctx.runMutation(internal.seed.insertCampaignActions, {
			campaignIds,
			supporterIds,
			orgIds
		});
		...
		console.log('[seed] Phase 21: Activating compact recipient metrics...');
		await ensureRecipientMetricsReadyAfterSeed(ctx);

		console.log('[seed] Phase 22: Activating compact session authority...');
		await ensureSessionAuthorityReadyAfterSeed(ctx);

		console.log('[seed] Phase 23: Activating revisioned coalition metrics...');
		await ensureCoalitionMetricsReadyAfterSeed(ctx);

		console.log('[seed] Phase 24: Activating immutable network charters...');
		await ensureNetworkChartersReadyAfterSeed(ctx);

		console.log('[seed] Phase 25: Activating indexed supporter browsing...');
		await ensureSupporterBrowseReadyAfterSeed(ctx);

		console.log('[seed] Phase 26: Activating compact supporter audience actions...');
		await ensureSupporterAudienceReadyAfterSeed(ctx);
...
export const seedPublic = internalAction({
	...
	handler: async (ctx) => {
		...
		console.log('[seedPublic] Phase 6: Activating compact recipient metrics...');
		await ensureRecipientMetricsReadyAfterSeed(ctx);

		console.log('[seedPublic] Phase 7: Activating compact session authority...');
		await ensureSessionAuthorityReadyAfterSeed(ctx);

		console.log('[seedPublic] Phase 8: Activating empty coalition metrics...');
		await ensureCoalitionMetricsReadyAfterSeed(ctx);

		console.log('[seedPublic] Phase 9: Activating empty network charter projection...');
		await ensureNetworkChartersReadyAfterSeed(ctx);

		console.log('[seedPublic] Phase 10: Activating empty supporter browser...');
		await ensureSupporterBrowseReadyAfterSeed(ctx);

		console.log('[seedPublic] Phase 11: Activating empty supporter audience actions...');
		await ensureSupporterAudienceReadyAfterSeed(ctx);
...
export const zeroTemplateMetrics = internalMutation({
	...
	handler: async (ctx, { cursor, scheduleContinuation }) => {
		// One template per transaction keeps this maintenance path safe even for a
		// near-1 MiB legacy template plus its compact projection.
		const page = await ctx.db
			.query('templates')
			.order('asc')
			.paginate({ cursor: cursor ?? null, numItems: 1 });
		const templates: Array<Doc<'templates'>> = page.page;
		let patched = 0;
		let publicPatched = false;
		for (const t of templates) {
			const needsReset =
				t.verifiedSends !== 0 ||
				t.uniqueDistricts !== 0 ||
				t.deliveredDistricts === undefined ||
				t.deliveredDistricts.length !== 0;
			...
			await ctx.db.patch(t._id, templatePatch);
			await syncTemplateListProjection(ctx, { ...t, ...templatePatch });
			patched++;
			if (t.status === 'published' && t.isPublic) {
				await syncCompactPublicDiscoveryProjection(ctx, { ...t, ...templatePatch });
				publicPatched = true;
			}
		}
		if (publicPatched) {
		...
		if (!page.isDone && scheduleContinuation !== false) {
			await ctx.scheduler.runAfter(0, zeroTemplateMetricsRef, {
				cursor: page.continueCursor
			});
		}
		console.log(`[seed] Zeroed metrics on ${patched}/${templates.length} templates in this page.`);
		return {
	...
	handler: async (ctx, { userIds, orgIds }) => {
		...
		for (const orgId of orgIds) {
			const memberships = await ctx.db
				...
				.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
			assertSeedReconciliationBound('orgMemberships', memberships.length);
	...
	handler: async (ctx, { orgIds, templateIds }): Promise<Id<'campaigns'>[]> => {
		...
		for (const orgId of orgIds) {
			const orgCampaigns = await ctx.db
				...
				.withIndex('by_orgId', (q) => q.eq('orgId', orgId))
				.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
			assertSeedReconciliationBound('campaigns', orgCampaigns.length);
...
export const insertSupporterBatch = internalMutation({
	...
	handler: async (ctx, { supporters, orgId, orgIdx }): Promise<Id<'supporters'>[]> => {
		...
		for (const s of supporters) {
			...
			coalitionPairs.push({
				before: null,
				after: { _id: id, country: s.country, verified: s.verified }
			});
		...
		await applyCoalitionSupporterTransitionsBatch(ctx, orgId, coalitionPairs);
...
export const insertSegments = internalMutation({
	...
	handler: async (ctx, { orgIds, userIds }) => {
		...
		const countSegment = async (orgId: Id<'organizations'>, tagName: string): Promise<number> => {
			const supporters = await ctx.db
				.query('supporters')
				.withIndex('by_orgId', (q) => q.eq('orgId', orgId))
				.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
			assertSeedReconciliationBound('supporters', supporters.length);
			const verifiedIds = new Set(supporters.filter((s) => s.verified).map((s) => s._id));
			...
			const tagged = await ctx.db
				...
				.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
			assertSeedReconciliationBound('supporterTags', tagged.length);
		...
		const eventDefs = [
			{
				...
				description:
					'Join us for a town hall with local climate policy experts. Hear about the Clean Energy Investment Act and how you can make your voice heard.',
				eventType: 'HYBRID',
			...
			},
			{
				...
				description:
					'Help us call voters in key districts to remind them about upcoming registration deadlines.',
				eventType: 'VIRTUAL',
			...
			},
			{
				...
				description:
					'Walk with your neighbors to show support for community-based safety programs and demand funding in the city budget.',
				eventType: 'IN_PERSON',
			...
			},
			{
				...
				description:
					'An introductory webinar on the Clean Energy Investment Act — what it does, who it helps, and how to take action.',
				eventType: 'VIRTUAL',
				startAt: daysAgo(3),
...
export const insertDonations = internalMutation({
	...
	handler: async (ctx, { orgIds, campaignIds, supporterIds }) => {
		...
		for (const d of donationDefs) {
			...
			await applyDonationConfirmationTransition(ctx, null, donationValue, now);
			await ctx.db.insert('donations', donationValue);
		}
		...
		const completedCount = donationDefs.filter(
			(d) => d.campaignIdx === 3 && d.status === 'completed'
		).length;
		...
		const workflowDefs = [
			{
				...
				description:
					"Sends a welcome email when a new supporter joins, adds the 'volunteer' tag after 3 days.",
				trigger: { type: 'supporter_created', conditions: {} },
				steps: [
					{
						type: 'send_email',
						config: { subject: 'Welcome to Climate Action Now!', templateKey: 'welcome' },
						delay: 0
					},
			...
			},
			{
				...
				description:
					'Sends a follow-up email 1 day after an event ends, with a link to the campaign.',
				...
				steps: [
					{ type: 'delay', config: { days: 1 }, delay: 86_400_000 },
					{
						type: 'send_email',
						config: {
							subject: "Thanks for attending! Here's how to keep the momentum going.",
							templateKey: 'event_followup'
						},
						delay: 0
					}
				],
...
export const insertWorkflowExecutions = internalMutation({
	...
	handler: async (ctx, { workflowIds, supporterIds }) => {
		...
		for (const e of executionDefs) {
			await incrementWorkflowExecutionCount(ctx, workflowIds[e.wfIdx]);
...
export const insertEmailBlasts = internalMutation({
	...
	handler: async (ctx, { orgIds, campaignIds }) => {
		...
		await ctx.db.insert('emailBlasts', {
			...
			updatedAt: daysAgo(5),
			isAbTest: false,
			batches: [{ batchIndex: 0, status: 'sent', sentCount: 8, failedCount: 0, sentAt: daysAgo(5) }]
		});
...
export const insertNetwork = internalMutation({
	...
	handler: async (ctx, { orgIds }) => {
		...
		const networkId = await ctx.db.insert('orgNetworks', {
			...
			description:
				'A coalition of progressive organizations working together on climate, voting rights, and community safety.',
			ownerOrgId: orgIds[0],
			status: 'active',
		...
		// Schedule from the final roster state. The continuation runs after this
		// transaction commits and therefore snapshots all three seed members.
	...
	handler: async (ctx, { orgIds }) => {
		...
		for (const inv of inviteDefs) {
			...
			const hashBuf = await crypto.subtle.digest(
				'SHA-256',
				encoder.encode('invite-token:' + inv.token)
			);
	...
	handler: async (ctx, { campaignIds, supporterIds, orgIds }) => {
		...
		for (const lc of letterCampaigns) {
			...
			for (let i = 0; i < lc.supporterCount; i++) {
				...
				if (!action) throw new Error('SEED_CAMPAIGN_ACTION_INSERT_MISSING');
				await applyCoalitionActionTransition(ctx, null, action);
				await applySupporterAudienceActionTransition(ctx, null, action);
				actionCountTotal++;
...
export const insertDebates = internalMutation({
	...
	handler: async (ctx, { templateIds, suppressDiscoveryRefresh, coordinatedRebuildToken }) => {
		if (suppressDiscoveryRefresh) {
			await assertPublicDiscoveryCoordinatedRebuildAuthorized(ctx, coordinatedRebuildToken);
		}
		const now = Date.now();
		...
		const argumentDefs = [
			{
				stance: 'SUPPORT',
				body: 'This proposal addresses a critical need and has broad community backing. The evidence supports immediate action.',
				stakeAmount: 100
			},
			{
				stance: 'OPPOSE',
				body: 'While the intent is good, the implementation details are insufficient. We need more specific language before committing resources.',
				stakeAmount: 80
			},
			{
				stance: 'AMEND',
				body: 'The core proposal is sound but should include sunset provisions and measurable benchmarks for success.',
				stakeAmount: 60
			},
			{
				stance: 'SUPPORT',
				body: 'Strong precedent from other jurisdictions shows this approach works. We should move forward decisively.',
				stakeAmount: 50
			}
		];
		...
		for (let dIdx = 0; dIdx < debateDefs.length; dIdx++) {
			...
			const matchingCampaign = await ctx.db
				.query('campaigns')
				.withIndex('by_templateId_debateId', (q) =>
					q.eq('templateId', templateIds[d.templateIdx]).eq('debateId', undefined)
				)
				.first();
			if (matchingCampaign) {
...
const SEED_REPS: SeedRep[] = [
	{
		fullName: 'Sen. Maria Lopez',
		firstName: 'Maria',
		lastName: 'Lopez',
		title: 'U.S. Senator',
		email: 'lopez@senate.gov',
		party: 'D',
		district: 'CA',
		jurisdiction: 'CA',
	...
	{
		fullName: 'Sen. James Park',
		firstName: 'James',
		lastName: 'Park',
		title: 'U.S. Senator',
		email: 'park@senate.gov',
		party: 'D',
		district: 'CA',
		jurisdiction: 'CA',
		jurisdictionLevel: 'federal',
	...
	{
		fullName: 'Sup. Aaron Peskin',
		firstName: 'Aaron',
		lastName: 'Peskin',
		title: 'Supervisor',
		email: 'peskin@sfgov.org',
		district: 'D3',
	...
	{
		fullName: 'Sup. Hillary Ronen',
		firstName: 'Hillary',
		lastName: 'Ronen',
		title: 'Supervisor',
		email: 'ronen@sfgov.org',
		district: 'D9',
		jurisdiction: 'San Francisco',
		jurisdictionLevel: 'municipal',
		type: 'legislator'
	...
	{
		fullName: 'Sup. Dean Preston',
		firstName: 'Dean',
		lastName: 'Preston',
		title: 'Supervisor',
		email: 'preston@sfgov.org',
		district: 'D5',
		jurisdiction: 'San Francisco',
		jurisdictionLevel: 'municipal',
		type: 'legislator'
	...
	{
		fullName: 'Sup. Matt Dorsey',
		firstName: 'Matt',
		lastName: 'Dorsey',
		title: 'Supervisor',
		email: 'dorsey@sfgov.org',
		district: 'D6',
		jurisdiction: 'San Francisco',
		jurisdictionLevel: 'municipal',
		type: 'legislator'
		...
		for (const orgId of orgIds) {
			...
			for (const inv of invites) {
				if (
					inv.encryptedEmail &&
					inv.encryptedEmail !== '' &&
					!inv.encryptedEmail.startsWith('seed-')
				)
					continue;
...
export const getOrgDonations = internalQuery({
	...
	handler: async (ctx, { orgId }) => {
		const all = await ctx.db
			...
			.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
		assertSeedReconciliationBound('donations', all.length);
		return all.map((doc) => {
			...
			return {
				...
				rawName:
					d.name ?? (d.encryptedName && !d.encryptedName.startsWith('{') ? d.encryptedName : null)
			};
		});
	}
});
...
export const getOrgInvites = internalQuery({
	...
	handler: async (ctx, { orgId }) => {
		const all = await ctx.db
			...
			.withIndex('by_orgId', (idx) => idx.eq('orgId', orgId))
			.take(SEED_RECONCILIATION_ROW_LIMIT + 1);
		assertSeedReconciliationBound('orgInvites', all.length);
		return all.map((i) => ({
			_id: String(i._id),
...
export const patchSeedRecord = internalMutation({
	...
	handler: async (ctx, { table, id, patch }) => {
		...
		if (table === 'supporters') {
			const supporterId = normalizedId as Id<'supporters'>;
			const before = await ctx.db.get(supporterId);
			if (!before) throw new Error('PATCH_SUPPORTER_NOT_FOUND');
			await ctx.db.patch(supporterId, patch);
			await syncSupporterBrowseProjection(ctx, supporterId);
			await applyCoalitionSupporterTransition(ctx, before.orgId, before, {
				...before,
				...(patch as Partial<Doc<'supporters'>>)
			});
			return;
		}
...
const SEED_TABLES = [
	...
	// Close People readers before deleting tag links, tags, or supporters.
	'supporterBrowseMigrations',
	// Close coalition/charter readers before deleting their raw or compact
	// inputs. Source rows are drained before per-org projections; generation
	// rows are drained before their network/member parents.
	'coalitionMetricsMigrations',
	'networkCharterMigrations',
	...
	'eventRsvps',
	'events',
	// Receipt/event/A-B projection rows all reference emailBlasts → children first.
	'emailDeliveryReceipts',
	'emailEvents',
	'emailAbWinnerCandidates',
	'emailAbTestCohorts',
	'emailBlasts',
	'campaignDeliveries',
	'campaignActions',
	...
	'communityFieldContributions',
	'templateEndorsements',
	'segments',
	'supporterTags',
	'tags',
	'supporters',
	'coalitionOrgPressureBillInputs',
	'coalitionOrgPressureInputs',
	'coalitionOrgMetricDimensions',
	'coalitionOrgMetricInputs',
	'coalitionNetworkPressureBills',
	'coalitionNetworkPressureRows',
	'coalitionNetworkMetricDimensions',
	'coalitionNetworkRebuilds',
	'coalitionNetworkAggregates',
	'publicNetworkCharters',
	'orgInvites',
	'orgResolvedContacts',
	'orgNetworkMembers',
	'orgNetworks',
	...
	'revocationFlags',
	'revocationHaltAuditLog',
	'revocationReconcileState',
	...
	'messageGenerationJobs',
	'passkeyCeremonySessions',
	'sessionAuthorityMigrations',
	'userSessionAuthorities',
	'sessions',
	'accounts',
	'users'
] as const;

type SeedTable = (typeof SEED_TABLES)[number];

/**
 * Keep destructive maintenance transactions comfortably below Convex's read
 * and write limits even when a table contains unusually large documents.
 * Templates get their own one-row budget because a single template can be
 * close to 1 MiB and its compact projection/vector must be deleted in the same
 * transaction.
 */
const SEED_CLEAR_BATCH_SIZE = 4;
const SEED_TEMPLATE_CLEAR_BATCH_SIZE = 1;
const SEED_CLEAR_MAX_PAGES_PER_TABLE = 100_000;

type SeedTableClearPageResult = {
	deleted: number;
	failed: number;
	isDone: boolean;
	stage:
		| 'rows'
		| 'templateListProjections'
		| 'publicDetails'
		| 'compactSources'
		| 'compactTopics'
		| 'complete';
...
/**
 * Drain one table through bounded mutation transactions. Each page deletes
 * from the current head instead of advancing a cursor: deleting the page can
 * therefore never invalidate a continuation cursor or skip a surviving row.
 * A table is complete only after a separate transaction observes it empty.
 */
async function drainSeedTable(
	ctx: ActionCtx,
	table: SeedTable,
	coordinatedRebuildToken: string
): Promise<SeedTableDrainResult> {
	let deleted = 0;
	for (let pages = 1; pages <= SEED_CLEAR_MAX_PAGES_PER_TABLE; pages++) {
		let result: SeedTableClearPageResult;
		try {
			result = await ctx.runMutation(internal.seed.clearTable, {
				table,
			...
			return { deleted, failed: 1, pages, isDone: false, error };
		}
		...
		if (result.failed > 0) {
			return {
				...
				pages,
				isDone: false,
				error: `page-stage=${result.stage}`
			};
		}
		if (result.isDone) {
			return { deleted, failed: 0, pages, isDone: true };
		}
	}
	...
	return {
		deleted,
		failed: 1,
		pages: SEED_CLEAR_MAX_PAGES_PER_TABLE,
		isDone: false,
		error: `CLEAR_TABLE_PAGE_LIMIT_EXCEEDED:table=${table}:limit=${SEED_CLEAR_MAX_PAGES_PER_TABLE}`
	};
}
...
export const clearSeed = internalAction({
	...
	handler: async (
		ctx
	): Promise<{ tables: number; deleted: number; failedRows: number; failedTables: number }> => {
		...
		for (const table of SEED_TABLES) {
			const result = await drainSeedTable(ctx, table, coordinatedRebuildToken);
			totalDeleted += result.deleted;
			totalFailed += result.failed;
			if (!result.isDone || result.failed > 0) {
				failedTables++;
				console.error(
					`[seed] ${table} drain incomplete after ${result.pages} page(s): ${result.error ?? `failedRows=${result.failed}`}`
				);
			}
		}
		if (totalFailed > 0 || failedTables > 0) {
			console.error(
...
export const clearTable = internalMutation({
	...
	handler: async (
	...
	): Promise<SeedTableClearPageResult> => {
		if (!SEED_TABLES.includes(table as SeedTable)) {
		...
		const docs = await (ctx.db as any)
			.query(table)
			.order('asc')
			.take(table === 'templates' ? SEED_TEMPLATE_CLEAR_BATCH_SIZE : SEED_CLEAR_BATCH_SIZE);
		const endorsementTemplateIds: Id<'templates'>[] =
			table === 'templateEndorsements'
				? [...new Set((docs as Doc<'templateEndorsements'>[]).map((doc) => doc.templateId))]
				: [];
		let deleted = 0;
		let failed = 0;
		const errors: string[] = [];
		for (const doc of docs) {
			try {
				if (table === 'templates') {
					await deleteCompactPublicDiscoverySource(ctx, doc._id as Id<'templates'>);
					await deleteTemplateListProjection(ctx, doc._id as Id<'templates'>);
				}
				if (table === 'campaignActions') {
					const action = doc as Doc<'campaignActions'>;
				...
				if (table === 'supporters') {
					const supporter = doc as Doc<'supporters'>;
					await applyCoalitionSupporterTransition(ctx, supporter.orgId, supporter, null);
				...
				// Compact-source deletion and canonical template deletion are one
				// invariant, not two best-effort maintenance steps. Re-throw so Convex
				// rolls the whole templates-table mutation back if either side fails.
				if (table === 'templates') throw err;
		...
		// A direct bounded endorsement clear is itself a canonical counter writer.
		// Reconcile after the deletes inside this same transaction so neither the
		// template nor either compact plane can commit a stale total.
		if (table === 'templateEndorsements' && deleted > 0 && !suppressDiscoveryRefresh) {
			for (const templateId of endorsementTemplateIds) {
				const template = await ctx.db.get(templateId);
				if (!template) continue;
				const exactCount = await boundedExactEndorsementCount(ctx, templateId);
				if (exactCount === null) {
					// More than the bounded exact-repair window remains. Mark the
					// canonical counter non-authoritative and remove the compact public
					// source instead of rolling this deletion page back or publishing a
					// stale count. Later pages make progress from the table head; once
					// <=500 remain, the exact branch below restores both projections.
					await ctx.db.patch(templateId, { endorsementCount: undefined });
					if (template.status === 'published' && template.isPublic) {
						await deleteCompactPublicDiscoverySource(ctx, templateId);
					}
					continue;
				}
				const projectedTemplate = { ...template, endorsementCount: exactCount };
				await ctx.db.patch(templateId, { endorsementCount: exactCount });
				await syncTemplateListProjection(ctx, projectedTemplate);
				if (template.status === 'published' && template.isPublic) {
					await syncCompactPublicDiscoveryProjection(ctx, projectedTemplate);
				}
			}
		}
		...
		const sourceFamilies =
			PUBLIC_DISCOVERY_SOURCE_FAMILIES[table as keyof typeof PUBLIC_DISCOVERY_SOURCE_FAMILIES];
		...
		if (failed > 0) {
			return { deleted, failed, isDone: false, stage: 'rows' };
		}
		// Require an explicit empty observation on the next page. Besides making
		// completion auditable, this prevents an exact-size final page from being
		// mistaken for a complete drain.
		if (docs.length > 0) {
			return { deleted, failed: 0, isDone: false, stage: 'rows' };
		}

		if (table === 'templates') {
			const listProjections = await ctx.db
				.query('templateListProjections')
				.order('asc')
				.take(SEED_CLEAR_BATCH_SIZE);
			if (listProjections.length > 0) {
				for (const row of listProjections) {
					await deleteTemplateListProjection(ctx, row.templateId);
				}
				return {
					deleted: 0,
					failed: 0,
					isDone: false,
					stage: 'templateListProjections'
				};
			}

			const publicDetails = await ctx.db
				.query('publicTemplateDetailProjections')
				.order('asc')
				.take(SEED_CLEAR_BATCH_SIZE);
			if (publicDetails.length > 0) {
				for (const row of publicDetails) {
					await deleteCompactPublicDiscoverySource(ctx, row.templateId);
				}
				if (!suppressDiscoveryRefresh && sourceFamilies) {
					await invalidatePublicDiscoveryAfterDestructiveSourceChange(ctx, sourceFamilies);
				}
				return { deleted: 0, failed: 0, isDone: false, stage: 'publicDetails' };
			}

			// Canonical templates are gone. Drain any pre-existing compact orphans in
			// bounded pages. The helper removes the source/vector pair together for a
			// template id; processing sources first and topics second guarantees that
			// neither table can hide rows past a fixed slice boundary.
			const compactSources = await ctx.db
				.query('publicTemplateDiscoverySources')
				.order('asc')
				.take(SEED_CLEAR_BATCH_SIZE);
			if (compactSources.length > 0) {
				for (const row of compactSources) {
					await deleteCompactPublicDiscoverySource(ctx, row.templateId);
				}
				if (!suppressDiscoveryRefresh && sourceFamilies) {
					await invalidatePublicDiscoveryAfterDestructiveSourceChange(ctx, sourceFamilies);
				}
				return { deleted: 0, failed: 0, isDone: false, stage: 'compactSources' };
			}

			const compactTopics = await ctx.db
				.query('publicTemplateTopicVectors')
				.order('asc')
				.take(SEED_CLEAR_BATCH_SIZE);
			if (compactTopics.length > 0) {
				for (const row of compactTopics) {
					await deleteCompactPublicDiscoverySource(ctx, row.templateId);
				}
				if (!suppressDiscoveryRefresh && sourceFamilies) {
					await invalidatePublicDiscoveryAfterDestructiveSourceChange(ctx, sourceFamilies);
				}
				return { deleted: 0, failed: 0, isDone: false, stage: 'compactTopics' };
			}
		}

		return { deleted: 0, failed: 0, isDone: true, stage: 'complete' };
	}
});
...
export const beginCoordinatedPublicDiscoveryRebuild = internalMutation({
	args: {
		...
		kind: v.optional(v.union(v.literal('clearSeed'), v.literal('reseedTemplates')))
	},
	handler: async (ctx, { coordinatedRebuildToken, kind }) => {
...
export const reseedTemplates = internalAction({
	...
	handler: async (ctx) => {
		...
		const clearForReseed = async (table: (typeof SEED_TABLES)[number]): Promise<void> => {
			...
			if (!result.isDone || result.failed > 0) {
				console.error(
					`[reseedTemplates] ${table} drain incomplete after ${result.pages} page(s): ${result.error ?? `failedRows=${result.failed}`}; coordinated lock retained.`
				);
				throw new Error(
					`RESEED_TEMPLATES_CLEAR_PARTIAL_FAILURE:table=${table}:failedRows=${result.failed}:pages=${result.pages}`
				);
			}
		};
		...
		// 3. Close recipient-metrics readers/writers first, then delete raw
		// contributions before their compact rows. This prevents an action racing
		// the destructive clear from leaving a marker without its aggregate.
		await clearForReseed('recipientMetricsMigrations');
		await clearForReseed('positionDeliveries');
		await clearForReseed('positionRegistrations');
		await clearForReseed('messages');
		await clearForReseed('templateMessageDistrictMetrics');
		await clearForReseed('templatePositionDistrictMetrics');
		await clearForReseed('templateRecipientMetrics');
		...
		console.log(
			`[reseedTemplates] Done: ${templateIds.length} templates, debates, campaigns reseeded.`
		);
	}
});
...
export const getSeedUserIds = internalQuery({
	...
	handler: async (ctx): Promise<Id<'users'>[]> => {
		...
		return users.flatMap((user) => (user ? [user._id] : []));
	}
});
...
export const getSeedOrgIds = internalQuery({
	...
	handler: async (ctx): Promise<Id<'organizations'>[]> => {
		const orgs = await Promise.all(
			SEED_ORGS.map((seedOrg) =>
				ctx.db
					.query('organizations')
					.withIndex('by_slug', (q) => q.eq('slug', seedOrg.slug))
					.unique()
			)
		);
		return orgs.flatMap((org) => (org ? [org._id] : []));
	}
});
...
export const backfillScopes = internalMutation({
	...
	handler: async (ctx, { offset = 0, scheduleContinuation }) => {
		if (!Number.isSafeInteger(offset) || offset < 0) {
			throw new Error('BACKFILL_SCOPES_INVALID_OFFSET');
		}
		const scopedSeedTemplates = SEED_TEMPLATES.flatMap((template) =>
			template.scopes ? [{ slug: template.slug, scopes: template.scopes }] : []
		);
		const entry = scopedSeedTemplates[offset];
		let patched = 0;
		let publicPatched = false;
		if (entry) {
			const template = await ctx.db
				...
				.unique();
			if (template && (!template.scopes || template.scopes.length === 0)) {
				await ctx.db.patch(template._id, { scopes: entry.scopes });
				if (template.status === 'published' && template.isPublic) {
					await syncCompactPublicDiscoveryProjection(ctx, { ...template, scopes: entry.scopes });
					publicPatched = true;
				}
				patched++;
			}
		}
		if (publicPatched) await markPublicDiscoveryListDirty(ctx, 'aggregate');
		const nextOffset = offset + 1;
		const isDone = nextOffset >= scopedSeedTemplates.length;
		if (!isDone && scheduleContinuation !== false) {
			await ctx.scheduler.runAfter(0, backfillScopesRef, { offset: nextOffset });
		}
		console.log(`[seed] Backfilled scopes on ${patched}/${entry ? 1 : 0} templates in this page.`);
		return {
			scanned: entry ? 1 : 0,
			patched,
			isDone,
