 ...
 * FREE_INDIVIDUAL_AUTHORED_PER_MONTH; paid individual tiers (Voice/Advocate)
 * raise ONLY this limit. Both the floor and the per-tier limits live in
 * ./lib/planLimits, the one plan table both runtimes read.
...
import { FREE_INDIVIDUAL_AUTHORED_PER_MONTH } from './lib/planLimits';

// The limit resolver lives with the plan table; re-exported here so the
// authoring-cap call sites read one module.
export { authoredLimitForPlan } from './lib/planLimits';
 ...
 *
 * `limit` defaults to FREE_INDIVIDUAL_AUTHORED_PER_MONTH so the free-floor
...
export function decideIndividualAuthoring(
	...
	now: number,
	limit: number = FREE_INDIVIDUAL_AUTHORED_PER_MONTH
	...
	const isFreeFloor = limit <= FREE_INDIVIDUAL_AUTHORED_PER_MONTH;
