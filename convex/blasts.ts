...
export const getEncryptedSupportersForBlast = query({
  ...
  handler: async (ctx, args) => {
    ...
    // at the count step.
    //
    // `recipientFilter` is persisted as `v.any()` on create/update
    // mutations, so an unchecked `as typeof filter` cast at read time
    // would let a single malformed write (e.g., saved
    ...
    // tagIds must be an array of strings; verified must be one of the
    // three literals; anything else is silently ignored (treat as
...
export const recordBlastReceiptsInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (blast.sendMode !== "client-direct" && blast.sendMode !== "tee-sealed") {
      throw new Error(
        `Receipts not supported for sendMode '${blast.sendMode ?? "(unset)"}'`,
      );
    }
    if (blast.status !== "sending" && blast.status !== "sent") {
      throw new Error(
        `Cannot record receipts for blast in status '${blast.status}'`,
      );
    }
    ...
    for (const r of args.receipts) {
      ...
      const existing = await ctx.db
        ...
        .withIndex("by_blastId_recipientEmailHash", (q) =>
          q
            .eq("blastId", args.blastId)
            .eq("recipientEmailHash", r.recipientEmailHash),
        )
        .first();
      if (existing) {
