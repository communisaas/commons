...
const getEventInternalRef = makeFunctionReference<'query'>(
	'events:getEventInternal'
	...
	Doc<'events'> | null
>;
const insertRsvpRef = makeFunctionReference<'mutation'>(
	'events:insertRsvp'
) as unknown as FunctionReference<
	'mutation',
	'internal',
...
export const list = query({
	...
	handler: async (ctx, args) => {
		...
		if (args.status) {
			q = ctx.db
				.query('events')
				.withIndex('by_orgId_status', (qb) => qb.eq('orgId', org._id).eq('status', args.status!));
		} else {
			q = ctx.db.query('events').withIndex('by_orgId', (qb) => qb.eq('orgId', org._id));
		}
...
export const update = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.requireVerification !== undefined)
			patch.requireVerification = args.requireVerification;
...
export const createRsvp = action({
	...
	handler: async (ctx, args): Promise<InsertRsvpResult> => {
		...
		if (!orgKey) {
			throw new Error(
				'Organization encryption not configured. An org owner must set up encryption before accepting RSVPs.'
			);
		}

		const encEmail = await encryptWithOrgKey(
		...
		if (args.name.trim()) {
			const encName = await encryptWithOrgKey(
				args.name.trim(),
				orgKey,
				`rsvp:${emailHash}`,
				'name'
...
export const checkIn = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.runMutation(internal.workflows.dispatchTrigger, {
			orgId: org._id,
			triggerType: 'event_checkin',
			supporterId: rsvp.supporterId,
			triggerEvent: {
				type: 'event_checkin',
				eventId: args.eventId,
				rsvpId: args.rsvpId,
...
export const publicCheckIn = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (
			event.checkinCode &&
			args.checkinCode &&
			args.checkinCode.length === event.checkinCode.length
		) {
			let mismatch = 0;
		...
		// the checkinCode.
		let checkedInRsvpId: Id<'eventRsvps'>;
		if (rsvp) {
			await ctx.db.patch(rsvp._id, {
				checkedInAt: Date.now(),
				attendanceVerified: verifiedTrust,
				attendanceVerificationMethod: verifiedTrust ? args.verificationMethod : undefined,
				attendanceIdentityCommitment: verifiedTrust ? args.identityCommitment : undefined,
				updatedAt: Date.now()
			});
			checkedInRsvpId = rsvp._id;
		} else {
			...
			checkedInRsvpId = await ctx.db.insert('eventRsvps', {
				eventId: args.eventId,
				encryptedEmail: '',
				emailHash: args.emailHash,
				status: 'GOING',
		...
		await ctx.runMutation(internal.workflows.dispatchTrigger, {
			...
			triggerEvent: {
				type: 'event_checkin',
				eventId: args.eventId,
				rsvpId: checkedInRsvpId,
				publicCheckin: true,
				verified: verifiedTrust,
				verificationMethod: verifiedTrust ? (args.verificationMethod ?? null) : null,
				timestamp: Date.now()
			}
		});
