...
const BATCH_SIZE = 50;
const BACKFILL_PAGE_SIZE_MAX = 100;
const BACKFILL_CURSOR_MAX_BYTES = 2_048;
const BACKFILL_PAGE_MAX_BYTES = 512 * 1024;
const NORMALIZER_PAGE_SIZE = 100;
const NORMALIZER_MAX_PAGES_PER_RUN = 20;
const NORMALIZER_MAX_BYTES_PER_PAGE = 512 * 1024;

type LegacySupporterPii = { name?: string; phone?: string; email?: string };
type LegacyDonationPii = { name?: string; email?: string };

function boundedBackfillPage(limit: number, cursor?: string): number {
	if (!Number.isSafeInteger(limit) || limit < 1 || limit > BACKFILL_PAGE_SIZE_MAX) {
		throw new Error('BACKFILL_PAGE_SIZE_INVALID');
	}
	if (cursor && new TextEncoder().encode(cursor).byteLength > BACKFILL_CURSOR_MAX_BYTES) {
		throw new Error('BACKFILL_CURSOR_INVALID');
	}
	return limit;
}
...
export const getSupportersNeedingBackfill = internalQuery({
	...
	handler: async (ctx, { orgId, paginationCursor, limit }) => {
		const pageSize = boundedBackfillPage(limit, paginationCursor);
		const result = await ctx.db
			...
			.paginate({
				...
				maximumBytesRead: BACKFILL_PAGE_MAX_BYTES
			});
		if (result.pageStatus === 'SplitRequired') throw new Error('BACKFILL_PAGE_SPLIT_REQUIRED');
...
export const getDonationsNeedingBackfill = internalQuery({
	...
	handler: async (ctx, { orgId, paginationCursor, limit }) => {
		...
		const needsWork = result.page.filter(
			(d) =>
				!d.encryptedEmail || d.encryptedEmail === '' || !d.encryptedName || d.encryptedName === ''
		);
...
export const backfillSupporterGlobalHashes = internalAction({
	...
	handler: async (ctx, { orgId, force }) => {
		...
		while (!isDone) {
			const batch = await ctx.runQuery(internal.backfill.getSupportersNeedingGlobalHash, {
				orgId,
				paginationCursor,
				limit: BATCH_SIZE
			});
			...
			for (const s of batch.items) {
				try {
					...
					const needsPhoneWork = !!encPhone && (force || !s.hasGlobalPhoneHash || !s.hasPhoneHash);
					...
					if (Object.keys(patch).length > 0) {
						...
						// its current state.
						const guard = await ctx.runMutation(internal.backfill.patchSupporterIfNotMoved, {
							supporterId: s._id,
					...
					console.error(`[backfillSupporterGlobalHashes] Failed on supporter ${s._id}:`, err);
					failed++;
				}
			}
		}
...
export const migrateEmailHashes = internalAction({
	...
	handler: async (ctx, { orgId }) => {
		...
		while (!isDone) {
			const batch = await ctx.runQuery(internal.backfill.getSupportersForHashMigration, {
				orgId,
				paginationCursor,
				limit: BATCH_SIZE
			});
...
export const normalizeBlastRecipientFilters = internalAction({
	...
	handler: async (
		...
		for (let pageNumber = 0; pageNumber < NORMALIZER_MAX_PAGES_PER_RUN; pageNumber++) {
			const result: {
				rows: Array<{ _id: Id<'emailBlasts'>; recipientFilter: unknown }>;
				continueCursor: string | null;
				isDone: boolean;
			...
			if (result.isDone) {
				const finalResult = { scanned, normalized, complete: true, continueCursor: null };
				console.log(`[normalizeBlastRecipientFilters] ${JSON.stringify(finalResult)}`);
				return finalResult;
			}
			continueCursor = result.continueCursor;
		}
		const result = { scanned, normalized, complete: false, continueCursor };
...
export const listEmailBlastsForFilterAudit = internalQuery({
	...
	handler: async (ctx, { cursor }) => {
		const page = await ctx.db.query('emailBlasts').paginate({
			numItems: NORMALIZER_PAGE_SIZE,
			cursor: cursor ?? null,
			maximumRowsRead: NORMALIZER_PAGE_SIZE + 1,
			maximumBytesRead: NORMALIZER_MAX_BYTES_PER_PAGE
		});
		return {
			rows: page.page.map((r) => ({
				_id: r._id,
				recipientFilter: r.recipientFilter as unknown
			})),
...
export const normalizeCampaignTemplateIds = internalAction({
	...
	handler: async (
		...
		for (let pageNumber = 0; pageNumber < NORMALIZER_MAX_PAGES_PER_RUN; pageNumber++) {
			const result: {
				rows: Array<{ _id: Id<'campaigns'>; templateId: string | undefined }>;
				continueCursor: string | null;
				isDone: boolean;
			...
			for (const row of result.rows) {
				...
				const normalized = await ctx.runQuery(internal.backfill.checkTemplateIdValid, {
				...
				await ctx.runMutation(internal.backfill.clearCampaignTemplateId, { id: row._id });
				cleared++;
			}
			if (result.isDone) {
				const finalResult = { scanned, cleared, valid, complete: true, continueCursor: null };
				console.log(`[normalizeCampaignTemplateIds] ${JSON.stringify(finalResult)}`);
				return finalResult;
			}
			continueCursor = result.continueCursor;
		}
		const result = { scanned, cleared, valid, complete: false, continueCursor };
...
export const normalizeSmsRecipientFilters = internalAction({
	...
	handler: async (
		...
		for (let pageNumber = 0; pageNumber < NORMALIZER_MAX_PAGES_PER_RUN; pageNumber++) {
			const result: {
				rows: Array<{ _id: Id<'smsBlasts'>; recipientFilter: unknown }>;
				continueCursor: string | null;
				isDone: boolean;
			...
			for (const row of result.rows) {
				if (!isConformingSmsRecipientFilter(row.recipientFilter)) {
					await ctx.runMutation(internal.backfill.clearSmsBlastFilter, { id: row._id });
					cleared++;
				}
			}
			if (result.isDone) {
				const finalResult = { scanned, cleared, complete: true, continueCursor: null };
				console.log(`[normalizeSmsRecipientFilters] ${JSON.stringify(finalResult)}`);
				return finalResult;
			}
			continueCursor = result.continueCursor;
		}
		const result = { scanned, cleared, complete: false, continueCursor };
...
export const listSmsBlastsForFilterAudit = internalQuery({
	args: { cursor: v.optional(v.union(v.string(), v.null())) },
	handler: async (ctx, { cursor }) => {
		const page = await ctx.db.query('smsBlasts').paginate({
			numItems: NORMALIZER_PAGE_SIZE,
			cursor: cursor ?? null,
			maximumRowsRead: NORMALIZER_PAGE_SIZE + 1,
			maximumBytesRead: NORMALIZER_MAX_BYTES_PER_PAGE
...
// at deploy time. Run the paginated operator driver BEFORE pushing to prod:
//   npx tsx scripts/audit-enum-conformance.ts
// Output: complete per-field counts grouped by value. Any value outside the
...
const CANONICAL_VALUES: Record<string, ReadonlyArray<string>> = {
	// C12 enums
	'campaigns.type': ['LETTER', 'EVENT', 'FORM', 'FUNDRAISER', 'CONGRESSIONAL'],
	...
	'subscriptions.plan': ['inactive', 'starter', 'organization', 'coalition', 'voice', 'advocate'],
	...
	'emailBlasts.status': ['draft', 'scheduled', 'sending', 'sent', 'failed', 'outcome_unknown'],
...
const ENUM_AUDIT_FIELDS = {
	campaigns: ['campaigns.type', 'campaigns.status'],
	events: ['events.eventType', 'events.status'],
	subscriptions: ['subscriptions.plan', 'subscriptions.status', 'subscriptions.paymentMethod'],
	emailBlasts: ['emailBlasts.status'],
	smsBlasts: ['smsBlasts.status'],
	smsMessages: ['smsMessages.status'],
	eventRsvps: ['eventRsvps.status'],
	debates: ['debates.status'],
	accountabilityReceipts: ['accountabilityReceipts.causalityClass']
} as const;

const enumAuditTableValidator = v.union(
	v.literal('campaigns'),
	v.literal('events'),
	v.literal('subscriptions'),
	v.literal('emailBlasts'),
	v.literal('smsBlasts'),
	v.literal('smsMessages'),
	v.literal('eventRsvps'),
	v.literal('debates'),
	v.literal('accountabilityReceipts')
);

export const auditEnumConformance = internalQuery({
	args: {
		table: enumAuditTableValidator,
	...
	handler: async (
		...
		switch (table) {
			case 'campaigns': {
				const result = await ctx.db.query('campaigns').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'events': {
				const result = await ctx.db.query('events').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'subscriptions': {
				const result = await ctx.db.query('subscriptions').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'emailBlasts': {
				const result = await ctx.db.query('emailBlasts').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'smsBlasts': {
				const result = await ctx.db.query('smsBlasts').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'smsMessages': {
				const result = await ctx.db.query('smsMessages').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'eventRsvps': {
				const result = await ctx.db.query('eventRsvps').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'debates': {
				const result = await ctx.db.query('debates').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
			case 'accountabilityReceipts': {
				const result = await ctx.db.query('accountabilityReceipts').paginate(paginationOpts);
				page = { ...result, page: result.page as unknown as Array<Record<string, unknown>> };
				break;
			}
		}

		const summary = ENUM_AUDIT_FIELDS[table].map((field) => {
			const property = field.slice(field.indexOf('.') + 1);
			return buildFieldAudit(field, page.page, (row) => row[property]);
		});
		return {
			summary,
			continueCursor: page.isDone ? null : page.continueCursor,
