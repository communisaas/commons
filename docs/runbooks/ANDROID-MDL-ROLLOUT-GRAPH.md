...
The existing `/verify-bridge` web flow remains in the codebase as a rollback/security
reference path, but it is no longer offered in the user-facing desktop flow.
...
- `A6j` direct QR desktop UI is complete in this delta: `/api/identity/direct-mdl/start` creates authenticated desktop direct sessions and emits an `openid4vp://authorize` QR payload after signer preflight; `/api/identity/direct-mdl/stream/{sessionId}` carries request-fetched/completed/failed/expired events back to the desktop; `/api/identity/direct-mdl/cancel` invalidates abandoned direct QR sessions; the desktop component shows requested mDL fields plus the server-derived account label, validates `credentialHash` and `identityCommitmentBound`, fails closed on invalid completions, cancels old direct sessions on retry/fallback/back/destroy, and originally kept the guided `/verify-bridge` path available.
...
- `A6k` staging preflight completed at `1498e9df`: `main`, `staging`, and `production`
  were aligned; CI/deploy green; `staging.commons.email` was verified as a proxied CNAME to
  `staging.communique-site.pages.dev`; operator-shell health/readiness probes passed; production
  direct QR remained intentionally disabled.
- `A6l` first bridge-removal cut is complete in this delta: `/verify-bridge` and the
...
- The request object contains a user-visible account signal where the wallet/OS profile can
  surface it. If physical smoke shows no meaningful account context, direct QR cannot become
  default outside staging until a Brutalist-reviewed mitigation is accepted.
- `/verify-bridge` remains available only as a rollback/security-reference backend while A7
  decides whether to hard-gate or delete it.
...
- The `Configure Cloudflare Branch Alias` workflow has verified or updated
  ...
  verified the latest staging Pages deployment and branch alias target for the reviewed
  commit without the deployment moving during DNS verification. If the GitHub runner is
  challenged by Cloudflare, treat its custom-domain probes as advisory and use the
  operator-shell probes below as the gate.
...
`/verify-bridge` backend rollback/reference gate (`A7`) passes only when one reviewed outcome
is true:

- Public bridge entrypoints are deleted or feature-gated off by default and covered by
  route tests.
- Any explicitly retained rollback path is operator-gated, not offered in normal user
  flow, binds phone claim to the displayed pairing code, rejects duplicate/stale completions,
  and expires KV bridge sessions within the configured TTL.
