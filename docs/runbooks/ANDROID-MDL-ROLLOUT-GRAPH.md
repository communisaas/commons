...
| A3a | impl-done | Commit OpenID4VP issuer trust hardening: reject header `jwk`, require `x5c`, anchor leaf certs to IACA roots. | `oid4vp-verify` and `cose-verify`. | Brutalist security review on issuer trust only. |
| A3b | active | Remove `SKIP_ISSUER_VERIFICATION` from production code paths; replace remaining synthetic-token tests with mocks/fixtures. | `oid4vp-verify` without env bypass reliance. | Brutalist security review on bypass removal. |
| A3c | impl-done | Commit SD-JWT disclosure hash binding. | `oid4vp-verify` SD-JWT disclosure tests. | Brutalist security review on selective-disclosure handling only. |
...
| A5 | queued | Add credential-hash reuse detection/cooldown for Android OID4VP replay defense in depth. | Unit test duplicate credential hash handling across users/sessions. | Brutalist security review on replay window. |
