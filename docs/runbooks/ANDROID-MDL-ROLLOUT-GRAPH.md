...
| A6d-b | active | Accept Android `mso_mdoc` safely: verify DeviceAuth over the OpenID4VP DC API handover, decode `vp_token.mdl[]` DeviceResponse, then pass only verified data through the privacy boundary. | DeviceAuth replay regression, `oid4vp-verify`, `mdl-mdoc`, DC API mdoc fixture tests. | Brutalist security review before commit; no live smoke until this passes. |
...
- Next tractable target is `A6d-b`: wire DeviceAuth verification for Android OpenID4VP `mso_mdoc` DeviceResponses using the stored origin and nonce, then keep unsupported/tampered responses fail-closed.
