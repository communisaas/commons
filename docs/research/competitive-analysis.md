...
| Letter campaigns / Power Landscape | Built (code); reach mostly latent, congressional send flag-off | Decision-maker resolution across the 24-slot schema; live structured coverage is congressional, state/local via agentic search, special-district slots un-ingested (see scorecard). Congressional CWC send is flag-off / transport-unarmed; production send path is customer-signing-gated. |
| Legislative intelligence (bills, votes, alerts) | Trail — substrate/stub only | Bill search + watch live; roll-call vote tracking is a no-op stub, bill embeddings unwired in the sync path, per-supporter alerts absent. Incumbent fortress — intentionally not built to parity. |
