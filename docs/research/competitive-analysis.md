...
FiscalNote's April 8, 2026 PolicyNote API launch added VoterVoice-powered address-to-district matching as a programmatic primitive, with explicit marketing language: *"Unlike competitors who rely on third-party vendors for district matching and inherit their limitations, FiscalNote owns and maintains its district data infrastructure."* This is market validation of the Shadow Atlas thesis — stated by an incumbent under going-concern pressure. VoterVoice covers federal/state/basic local; Shadow Atlas's 24-slot H3 architecture is the only owned boundary-data layer that can ever *host* special-district boundaries (water, fire, transit, etc.) at $0 marginal — a latent moat. Live structured coverage today is congressional only (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested.
...
**Shadow Atlas position:** a 24-slot H3 architecture, owned boundary-data infrastructure at $0 marginal, the only layer that can ever host special-district boundaries at all (latent today). Live structured coverage is congressional (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested. FiscalNote's April 2026 PolicyNote API launch explicitly markets district matching as a moat — *"Unlike competitors who rely on third-party vendors..."* — market validation under duress.
...
2. **Owned local + special-district reach (Shadow Atlas).** A 24-slot H3 architecture whose district resolution is $0 marginal (owned static data vs Cicero's per-lookup) — **the only one that *can* address special districts at all**. That breadth is mostly **latent** today: live structured coverage is **congressional** (the `cd` boundary bundles + officials the client actually consumes); state and local officials resolve via **agentic search** (a paid LLM/search step, not the $0 static layer), not structured boundary→official data; and the special-district slots (water/fire/transit/school/library/hospital/judicial — toward 39,555 districts / ~84K officials) are **built but not yet ingested (0 today)**. The durable moat is owning the only boundary-data layer that can *ever* host the special districts that Quorum Local (cities ≥3K pop, 0 special districts), Cicero (~400 cities, the $0.03–0.04/lookup monopoly), and VoterVoice (>250K population only) structurally cannot reach. The civic-data API collapse strengthens this reach moat; it does not make Commons a legislative-intelligence competitor.
...
| Local + special-district reach (Shadow Atlas) | **TRANSCEND (architecture); live ≈ congressional** | The 24-slot H3 schema serves owned boundary data at $0 marginal and is the only one that *can* host special-district boundaries — a durable but **latent** moat. Live structured coverage today is congressional (boundary + officials); state/local officials resolve via agentic search; the special-district slots are built but **not yet ingested (0)**. Cicero (federal+state, ~400 cities, 0 special districts) structurally can't represent them. The moat is architectural ownership + the API-collapse tailwind, not present-day breadth. |
...
Google Civic Info's shutdown forced every advocacy platform to either pay Cicero or scramble for alternatives. **Commons owns the district layer** — Shadow Atlas (94,166 districts, a 24-slot H3 architecture, R-tree spatial indexing, sub-50ms p95). Zero marginal cost for the owned boundary data; no third-party dependency. Live structured coverage today is congressional (house rep + two senators via congress-legislators); state/local via the paid agentic pipeline; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat.
...
Commons fills this vacuum structurally. Verification is orthogonal to ideology. The protocol doesn't check politics — it checks proof. A $10 Starter entry (with author-free onboarding) that includes email campaigns, verified advocacy, full uncapped API access, and district targeting — congressional live, state/local via agentic search, on a 24-slot schema that is the only one that can ever reach special districts — is immediately the best tool a conservative advocacy org has ever had. It's also the best tool many progressive orgs have ever had — the ones too small for AN's $15/mo or excluded from EveryAction's ecosystem for other reasons.
...
|---|---|---|---|
| **District matching** | Full street address → Cicero API (~$0.03/lookup) | Postal code → Shadow Atlas (94,166 districts, 24-slot H3 architecture, sub-50ms) | Zero marginal cost for the owned boundary data; no third-party dependency. Live structured officials coverage is congressional (house rep + two senators); state/local via the paid agentic pipeline; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat. |
| **Official lookup** | Static database of ~500K officials (Cicero, Quorum, VoterVoice) | Congressional officials (house rep + two senators) from congress-legislators; state/local via a 3-phase agentic enrichment pipeline | Live structured officials coverage is congressional only; state/local via agentic search. Special-district offices (water, transit, school board) un-ingested — architecture can reach them, slots empty today. |
...
| **Advocate mapping** | District overlay on map (Muster, VoterVoice, Quorum). Typically federal + state legislative only. | Shadow Atlas: a 24-slot H3 architecture per cell. H3-indexed. 94,166 districts. | The 24-slot schema is the only one that can ever address water, fire, school, transit, judicial districts — un-ingested today; live structured coverage congressional, state/local via agentic search. Latent moat. |
...
| District proof | None | Merkle proof against the hierarchical district tree (24-slot schema; live structured coverage congressional, special-district slots un-ingested) |
...
| List management / CRM | Upcoming | CSV import/export, tags, segments. Tier-aware. District membership across the 24-slot schema (live structured coverage congressional; state/local via agentic search; special-district slots un-ingested). |
...
| Shadow Atlas (district tree) | Built (architecture); reach mostly latent | 94,166 districts, a 24-slot H3 architecture per cell, H3-indexed. Live structured coverage congressional; state/local via agentic search; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat. |
...
| **Shadow Atlas (district layer)** | 94,166 districts, a 24-slot H3 architecture (21 slots with zero competitor coverage), R-tree spatial index. District resolution + congressional officials are owned static data at zero marginal cost (vs Cicero's per-lookup); state/local official enrichment runs the paid agentic pipeline. Live structured coverage is congressional (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested — the only schema that can ever host the 39,555 special districts' boundaries, a latent moat. | **Low** — Cicero charges $0.03–0.04/lookup; Google Civic shut down; the civic-data API collapse strengthens this. Building a competing district layer at this scope is a multi-year effort. The durable moat. |
...
3. **Local-government / special-district advocacy** — the reach wedge. School boards, water districts, transit authorities, fire districts, judicial circuits. The 24-slot Shadow Atlas schema is the only one that can ever address these — 21 of its 24 slots have **zero** competitor coverage — a latent moat: special-district slots un-ingested today, live structured reach congressional, local officials via agentic search. An add-to-Quorum at $75, not a switch.
...
| Local government | Sub-state + special-district targeting | Federal + state legislative only | A 24-slot schema (21 slots with zero competitor coverage) that can address school, water, fire, transit, judicial districts — latent, not live: slots un-ingested today, live structured reach congressional, local officials via agentic search |
