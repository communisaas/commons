...
2. **Owned local + special-district reach (Shadow Atlas).** 24 boundary types per H3 cell at $0 marginal cost — **a 24-slot H3 architecture — the only one that *can* address special districts at all** — at $0 marginal cost. That breadth is mostly **latent** today: live structured coverage is **congressional** (the `cd` boundary bundles + officials the client actually consumes); state and local officials resolve via **agentic search**, not structured boundary→official data; and the special-district slots (water/fire/transit/school/library/hospital/judicial — toward 39,555 districts / ~84K officials) are **built but not yet ingested (0 today)**. The durable moat is owning the only data layer that can *ever* cover, at $0, the special districts that Quorum Local (cities ≥3K pop, 0 special districts), Cicero (~400 cities, the $0.03–0.04/lookup monopoly), and VoterVoice (>250K population only) structurally cannot reach. The civic-data API collapse strengthens this reach moat; it does not make Commons a legislative-intelligence competitor.
...
| **Shadow Atlas (district layer)** | 94,166 districts, a 24-slot H3 architecture (21 slots with zero competitor coverage), R-tree spatial index. Owned data, zero marginal cost. Live structured coverage is congressional (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested — the only schema that can ever cover the 39,555 special districts / ~84K officials at $0, a latent moat. | **Low** — Cicero charges $0.03–0.04/lookup; Google Civic shut down; the civic-data API collapse strengthens this. Building a competing district layer at this scope is a multi-year effort. The durable moat. |
