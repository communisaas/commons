...
FiscalNote's April 8, 2026 PolicyNote API launch added VoterVoice-powered address-to-district matching as a programmatic primitive, with explicit marketing language: *"Unlike competitors who rely on third-party vendors for district matching and inherit their limitations, FiscalNote owns and maintains its district data infrastructure."* This is market validation of the Shadow Atlas thesis — stated by an incumbent under going-concern pressure. VoterVoice covers federal/state/basic local; the Shadow Atlas differentiator vs every incumbent (FiscalNote owns its district data too) is not a present cost edge and not ownership — it is a *latent* architectural moat: an owned 24-slot H3 architecture, the only one built to host special-district boundaries (water, fire, transit, etc.), empty today but positioned to be ingested as that data becomes available. The $0-per-lookup is only the congressional public-data floor (TIGER + congress-legislators — free public data available to Cicero and anyone, not a structural cost advantage), and the sold state/local/special-district segment resolves officials via a paid agentic search pipeline (Gemini + Exa + Firecrawl) that plausibly costs more per lookup than Cicero's $0.03–0.04, so there is no present per-lookup cost win there. The durable bet is the owned architecture + the API-collapse timing + the path to $0 special-district resolution once those slots are ingested; live structured coverage today is congressional (house rep + two senators via congress-legislators); the 23 non-congressional slots — the local-government layers plus the special districts (slots 11–23) — are un-ingested.
...
**Cicero (Melissa Data) is now the effective monopolist** for address→district at $0.03–0.04/lookup. A medium campaign (50K supporters) = $1,500–$2,000/campaign in Cicero fees; 10 campaigns/year = $15K–$20K/year just for district resolution. Cicero offers volume discounts and 1,000 free trial credits, so no vendor is simply undercut on price; Shadow Atlas's edge is not a present per-lookup cost structure but a *latent* architectural one — the only owned 24-slot architecture built to host special-district boundaries Cicero is not built to represent, empty today and positioned to be ingested as the API layer dies around it. The $0-per-lookup is only the congressional public-data floor (free TIGER + congress-legislators, available to Cicero too); state and local officials resolve via a paid agentic search pipeline that plausibly costs more per lookup than Cicero, so the present cost advantage is the timing window and the path to $0 on ingestion, not a structural per-lookup edge. Special-district breadth is latent (un-ingested).
...
**Shadow Atlas position:** an owned 24-slot H3 architecture whose differentiator vs Cicero is not ownership (Cicero owns its data too) and not a present cost edge but a *latent* architectural moat — the only architecture positioned to host special-district boundaries Cicero is not built to represent, empty today. The $0-per-lookup is only the congressional public-data floor (TIGER + congress-legislators, free public data available to Cicero too); state and local officials resolve via a paid agentic search pipeline that plausibly costs more per lookup than Cicero, so the present asset is the owned architecture + API-collapse timing + the path to $0 on ingestion. Live structured coverage today is congressional (house rep + two senators via congress-legislators); special-district slots un-ingested. FiscalNote's April 2026 PolicyNote API launch explicitly markets district matching as a moat — *"Unlike competitors who rely on third-party vendors..."* — market validation under duress.
...
1. **AI-native authoring-to-delivery.** An end-to-end loop — intent → ground → author → resolve-targets → deliver → report — where source-grounded composition tailored to the resolved decision-maker set is shipped and config-gated-live (Gemini + Exa stratified search + Firecrawl fetch + adversarial-source-first ranking). Generation grounding is *disabled* and the output source list is bound to that pre-verified pool, so no fabricated-metadata URL reaches output (the inline [n] markers in the body are not separately validated against the pool). Whether a cited source actually *supports* the sentence is not validated at all — it rests on the prompt plus pool-level credibility filtering — so this is a strong source-provenance guardrail, not a citation-correctness proof. **Every incumbent ships AI for the org user, not for the action** (Quincy summarizes bills for a lobbyist, Que helps a fundraiser, SmartCheck tunes subject lines, ActionBot is admin chat-with-data, New/Mode varies message copy). None ships grounded AI on the constituent's action.

2. **Latent special-district reach (Shadow Atlas).** An owned 24-slot H3 architecture — **the only one built to host special-district boundaries** — whose value is **latent** today, not a present cost or coverage edge. The $0-per-lookup is only the congressional public-data floor (the `cd` boundary bundles + congress-legislators officials the client actually consumes — free public data available to Cicero too), and the sold state/local/special-district segment resolves officials via **agentic search** (a paid Gemini + Exa + Firecrawl step that plausibly costs more per lookup than Cicero, not a $0 static layer). The special-district slots (water/fire/transit/school/library/hospital/judicial — toward 39,555 districts / ~84K officials) are **built but not yet ingested (0 today)** — empty schema capacity, not present reach. The durable moat is the owned architecture positioned to host the special-district boundaries that Quorum Local (cities ≥3K pop, 0 special districts), Cicero (~400 cities, the $0.03–0.04/lookup monopoly), and VoterVoice (>250K population only) are not built to reach, welded to the API-collapse timing and the path to $0 special-district resolution once ingested. The civic-data API collapse strengthens this latent reach moat; it does not make Commons a legislative-intelligence competitor.
...
- **Congressional delivery — dark today.** Congressional send is gated by a code composite — FEATURES.CONGRESSIONAL (default false), the Convex CONGRESSIONAL_DELIVERY_LAUNCHED flag, CWC House/Senate transport env vars, and a per-submission ZK trust-tier floor of 4 — opened by operator env provisioning at customer signing, not by editing code. Today none are armed, so zero congressional messages can be delivered; own-rep routing is congressional-only and dark.
...
|---|---|---|
| AI-native authoring-to-delivery (grounded, set-tailored) | **TRANSCEND (authoring); delivery partial** | Source-grounded generation shipped + config-gated-live (Gemini + Exa stratified search + Firecrawl + adversarial-source-first ranking; grounding DISABLED so citations draw only from a pre-verified pool). No incumbent fetches + credibility-ranks external sources; all ship AI for the org user, not the action. The live send transport is mailto-based: congressional templates route through a mailto relay to congress@commons.email, narrowing at runtime to mailto or email_attested (Tier 2+ verified); the five-value deliveryMethod enum (email / email_attested / certified / direct / cwc) is a stored product label, not the live transport. Congressional CWC delivery is gated/dark (see "Where Commons trails"). |
| Local + special-district reach (Shadow Atlas) | **TRANSCEND (architecture); live ≈ congressional** | The owned 24-slot H3 schema is the only one **built to host** special-district boundaries — a durable but **latent** moat, not a present cost or coverage edge. The $0-per-lookup is only the congressional public-data floor (free TIGER + congress-legislators, available to Cicero too); live structured coverage today is congressional (boundary + officials), state/local officials resolve via a paid agentic search pipeline that plausibly costs more per lookup than Cicero, and the special-district slots are built but **not yet ingested (0)**. Cicero (federal+state, ~400 cities, 0 special districts) is not built to represent them. The moat is the owned special-district-capable architecture + the API-collapse timing + the path to $0 on ingestion, not present-day breadth or a present per-lookup cost win. |
...
| Multi-channel send at scale (SMS/P2P/bulk email) | **TRAIL / CEDE** | Production send path opened by operator env provisioning at customer signing (not by editing code); A2P 10DLC $10K-floor; managed-P2P (ActBlue Field Tools, Hustle, GetThru) own this. Parity posture, not build-mandate. |
...
| Legislative intelligence (bills, roll-call votes, scorecards) | **TRAIL (incumbent fortress)** | Vote-tracker is a no-op stub; bill embeddings unwired in the sync path; scorecards render empty (readers exist, but no vote data or receipts to score). Bill search/watch endpoints are built, but the federal ingest (syncPipeline) is a speculative-tier cron off in pre-launch production, so no live bill data flows yet. Congressional send is gated by the code composite (FEATURES.CONGRESSIONAL default false + Convex CONGRESSIONAL_DELIVERY_LAUNCHED + CWC transport env + per-submission ZK trust-tier 4). Quorum/FiscalNote/CQ own deep bill/vote intelligence. Credibility-floor, not front door. |
...
The structural gap that leads is reach, authoring, and access — not verification. No platform combines (1) grounded AI *on the constituent's action* (incumbents ship AI for the org user only), (2) latent special-district reach: an owned 24-slot schema (slot 0 congressional ingested; the other 23 non-congressional slots un-ingested; the special districts, slots 11–23, are the part no competitor covers) that is the only one architected to host the special-district boundaries of the 39,555 special districts that Quorum Local, Cicero (~400 cities), and VoterVoice (>250K pop only) are not built to reach — that breadth is latent today, not a present cost or coverage edge (live structured reach is congressional off free public data, state/local officials via a paid agentic search pipeline, special-district slots un-ingested) — and (3) universal-ideology, gate-at-delivery access. Underneath that wedge sits an ambient credibility layer: no platform verifies constituent identity. Every platform — Action Network, Quorum, EveryAction, Muster, VoterVoice, NationBuilder — relies on self-reported addresses fed through third-party geocoding (Cicero API, formerly Google Civic Info). That verification layer is the lowest-replicability moat and no competitor has started it, but it is the watermark, not the headline: the category thesis that an office handles a verified packet differently is unproven, and the realistic floor today barely exceeds incumbents' heuristic ZIP→Cicero. Treat it as an ambient tiebreaker for distrust-sensitive segments, not the front door.
...
Google Civic Info's shutdown forced every advocacy platform to either pay Cicero or scramble for alternatives. **Commons resolves the *congressional* district layer at $0 per lookup off free public data** — Shadow Atlas (a 24-slot H3 architecture, R-tree spatial indexing, sub-50ms p95). The differentiator vs Cicero is not ownership (Cicero owns its data too) and not a present cost edge — that $0 is just the congressional public-data floor (TIGER + congress-legislators, free and available to Cicero too) — but a *latent* architectural moat: the owned 24-slot architecture is the only one built to host special-district boundaries Cicero is not built to represent. Live structured coverage today is congressional (house rep + two senators via congress-legislators); state/local officials resolve via a paid agentic pipeline that plausibly costs more per lookup than Cicero; special-district slots un-ingested — empty schema capacity whose value is realized on ingestion, the path to $0 special-district resolution, not present reach.
...
**Commons's architecture is built to address 24 boundary types per cell.** Today Shadow Atlas has ingested only TIGER/Line *congressional* boundary geometry (slot 0); the other 23 slots (state legislative, county, municipal, school district, voting tabulation district, and the special districts) are **empty**. Live structured officials coverage is congressional only (house rep + two senators via congress-legislators); state/local officials resolve via a paid agentic search pipeline; special-district slots un-ingested. The architecture (H3-indexed, 24 slots per cell) is designed to ingest special-district boundaries as they become available — state by state, LAFCO by LAFCO. Every boundary type ingested then becomes targetable. The 24-slot schema is the only architecture built to host special-district boundaries — a *latent* moat: owned, special-district-capable architecture + the API-collapse timing + the path to $0 special-district resolution on ingestion, not a present cost edge or present coverage and not merely a data-ownership claim.

**The market opportunity:** A platform that lets a transit advocacy group, a school parent coalition, or a water district accountability org author verified campaigns to their specific local officials — free to author, $10 Starter to deliver — would have zero competition. Not underpriced competition. Zero. This is the latent opportunity the owned architecture is built for — the durable bet, realized on ingestion — not a shipped reach edge today: the special-district slots are un-ingested (empty schema capacity, not delivered coverage), and local officials resolve via a paid agentic search pipeline.
...
Commons fills this vacuum structurally. Verification is orthogonal to ideology. The protocol doesn't check politics — it checks proof. A $10 Starter entry (with author-free onboarding) that includes email campaigns, verified advocacy, full uncapped API access, and district targeting — congressional live, state/local via a paid agentic search pipeline, on an owned 24-slot schema that is the only architecture built to host special-district boundaries (latent, un-ingested today) — is immediately the best tool a conservative advocacy org has ever had. It's also the best tool many progressive orgs have ever had — the ones too small for AN's $15/mo or excluded from EveryAction's ecosystem for other reasons.
...
**Current boundary geometry:** US congressional, state legislative (upper + lower), county, municipal, school district, voting tabulation district boundaries via Census Bureau TIGER/Line. Live structured *officials* coverage, however, is congressional only (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested.
...
|---|---|---|---|
| **District matching** | Full street address → Cicero API (~$0.03/lookup, volume discounts + 1,000 free trial credits) | Postal code → Shadow Atlas (24-slot H3 architecture, sub-50ms) | *Latent* architectural moat, not a present cost win. The $0-per-lookup is only the congressional public-data floor (free TIGER + congress-legislators, available to Cicero too — no structural cost advantage there); state/local officials resolve via a paid agentic pipeline (Gemini + Exa + Firecrawl) that plausibly costs more per lookup than Cicero and is its own third-party dependency. Special-district slots un-ingested — the owned 24-slot schema is the only architecture built to host their boundaries, value realized on ingestion. |
| **Official lookup** | Static database of ~500K officials (Cicero, Quorum, VoterVoice) | Congressional officials (house rep + two senators) from congress-legislators; state/local via a 3-phase agentic enrichment pipeline | Live structured officials coverage is congressional only; state/local officials via a paid agentic search pipeline. Special-district offices (water, transit, school board) un-ingested — the architecture is built to host their boundaries, slots empty today. |
| **Email campaigns** | WYSIWYG, A/B testing, scheduling, SES/Sendgrid | WYSIWYG, A/B, scheduling, SES. Grounded, set-tailored composition with the output source list bound to a pre-verified pool; segmentable by engagement tier + district. | Lead advantage is authoring quality: grounding-verified content whose output source list is bounded to the verified pool (inline [n] markers not separately validated). Verified-district segmentation rides along as an ambient credibility note ("Established-tier constituents in CA-12" is a provable filter), not the headline. |
| **Message personalization** | AI generates 100–300 variants (Quorum, CiviClick, VoterVoice SmartCheck) | Message Writer Agent: two-phase grounding-verified composition; the output source list is bound to a pre-verified pool | The output source list is bounded to the verified pool (no fabricated-metadata URL reaches output); the inline [n] markers in the text are not parsed or validated against the pool. Content is grounding-verified AND sender is identity-verified. |
...
| **List management / CRM** | Email-keyed records, tags, self-reported geography, engagement scores | Email-keyed records, tags, identity commitment binding, engagement tiers (on-chain), district membership (congressional today; the 24-slot schema built to extend) | Tiers are non-fakeable. District membership is cryptographically proven, not self-reported. Live structured coverage congressional; state/local officials via a paid agentic search pipeline; special-district slots un-ingested — latent capacity to host their boundaries. |
...
| **Advocate mapping** | District overlay on map (Muster, VoterVoice, Quorum). Typically federal + state legislative only. | Shadow Atlas: an owned 24-slot H3 architecture per cell. H3-indexed. | The 24-slot schema is the only architecture built to host water, fire, school, transit, judicial district boundaries — un-ingested today (empty schema capacity, not present coverage); live structured coverage congressional, state/local officials via a paid agentic search pipeline. Latent moat, realized on ingestion. |
...
| **Legislative intelligence (bills, votes, alerts)** | Quorum, FiscalNote, CQ, FastDemocracy, PolicyEngage | Trail — substrate/stub only | **Incumbent fortress; Commons trails here.** Bill search + watch endpoints are built and real, but the federal ingest that feeds them (syncPipeline) is a speculative-tier cron off in pre-launch production, so no live bill data flows yet; roll-call vote tracking is a no-op stub, bill embeddings are unwired in the sync path, and per-supporter bill alerts are absent (legislativeAlerts is org-scoped only). Quorum/FiscalNote/CQ own deep bill/vote/committee intelligence. This is a credibility floor, not the front door. |
...
| **Commons** | Message Writer, Subject Line, Decision-Maker Discovery agents + agentic delegation (spec'd) | Production (agents), spec'd (delegation) | **Grounded AI on the action**: source-grounded composition tailored to the resolved decision-maker set (the output source list is bound to a pre-verified pool — no fabricated-metadata URL reaches output; inline [n] markers not separately validated), multi-turn subject line refinement, and decision-maker resolution — congressional via congress-legislators, state/local via 3-phase agentic search, with the 24-slot schema the latent (un-ingested) capacity for special districts. Agentic delegation (spec'd): tier-gated authority, privacy-preserving memory. | Ambient — ZK provenance on every action |
...
|---|---|---|
| Message quality | Form text or 100-300 AI variants, no grounding | Source-grounded composition tailored to the resolved decision-maker set; the output source list is bound to a pre-verified pool (inline [n] markers not separately validated) |
| Right official | Self-reported ZIP → Cicero (federal + state only; often wrong, 15–20% of ZIPs span multiple CDs) | Congressional decision-makers (house rep + two senators) from congress-legislators; state/local via a paid agentic search pipeline. The 24-slot schema is built to address water, fire, transit, school, judicial districts no platform (Commons included) covers today — those slots are un-ingested (latent, not live). |
...
| District proof | None | Merkle proof against the hierarchical district tree (24-slot schema built to host special-district boundaries; live structured coverage congressional, special-district slots un-ingested) |
...
**What Commons is:** Not Action Network with verification bolted on. A different thing. The atom of the system is a *verified-yet-AI-authored civic action* — and what leads is authoring-to-delivery: grounded composition tailored to the resolved decision-maker set, addressed to the correct official — congressional via congress-legislators, state/local via agentic search, with the 24-slot schema the latent (un-ingested) capacity for special districts — sold to orgs as aggregation and reach across a base. Every feature — email, letters, list management, analytics — works differently because of that authoring + reach spine. Verification is the ambient property that makes the action trustworthy, not the headline.
...
| **Letter campaigns** | Congress + state legislatures. District from self-reported ZIP. No way to verify the sender lives there. | Live structured reach is congressional (house rep + two senators via congress-legislators); state/local officials via a paid agentic search pipeline; the 24-slot schema is built to host special-district boundaries (fire, water, transit, school, judicial), un-ingested today. District from postal→district resolution + mDL. Letter carries a ZK proof. The letter is a verified civic instrument, not a form submission. |
| **List management** | Tags, CSV import/export, self-reported geography. An org's list is a collection of unverified claims. | Tags, CSV import/export, engagement tiers (non-fakeable, on-chain), district membership across the 24-slot schema (live structured coverage congressional; state/local officials via a paid agentic search pipeline; the schema is built to host special-district boundaries, un-ingested). An org's list is a set of cryptographically attested relationships. |
...
| Letter campaigns / Power Landscape | Built (code); reach mostly latent, congressional send flag-off | Decision-maker resolution across the 24-slot schema; live structured coverage is congressional, state/local officials via a paid agentic search pipeline, the schema is built to host special-district boundaries (un-ingested) (see scorecard). Congressional send is gated by the code composite — FEATURES.CONGRESSIONAL (default false), the Convex CONGRESSIONAL_DELIVERY_LAUNCHED flag, CWC House/Senate transport env vars, and a per-submission ZK trust-tier floor of 4 — opened by operator env provisioning at customer signing, not by editing code; today none are armed. |
| Legislative intelligence (bills, votes, alerts) | Trail — substrate/stub only | Bill search + watch endpoints exist, but their federal ingest (syncPipeline) is a speculative-tier cron off in pre-launch production, so no live bill data flows yet; roll-call vote tracking is a no-op stub, bill embeddings unwired in the sync path, per-supporter alerts absent. Incumbent fortress — intentionally not built to parity. |
| Mass email (A/B, scheduling) | Upcoming | SES backend, MJML templates. Segmentable by tier + district. |
| ZK verification pipeline / tiers / debate markets | Built (ambient substrate) | 5 circuits, 4 depths, mDL parsing, browser-side proofs; engagement tiers; LMSR + AI panel. Ambient credibility layer beneath the authoring + reach spine, not the headline. Note: congressional send is gated by the code composite (FEATURES.CONGRESSIONAL default false + Convex CONGRESSIONAL_DELIVERY_LAUNCHED + CWC transport env + per-submission ZK trust-tier 4) and is dark today. |
...
| List management / CRM | Upcoming | CSV import/export, tags, segments. Tier-aware. District membership across the 24-slot schema (live structured coverage congressional; state/local officials via a paid agentic search pipeline; the schema is built to host special-district boundaries, un-ingested). |
...
| Shadow Atlas (district tree) | Built (architecture); reach mostly latent | An owned 24-slot H3 architecture per cell, H3-indexed. Live structured coverage congressional; state/local officials via a paid agentic search pipeline; special-district slots un-ingested — the 24-slot schema is the only architecture built to host their boundaries, a latent moat realized on ingestion. |
...
|---|---|---|
| **AI-native authoring-to-delivery spine** | End-to-end intent → ground → author → resolve-targets → deliver → report. Source-grounded composition tailored to the resolved decision-maker set (Exa + Firecrawl + adversarial-source-first ranking; grounding disabled so citations draw from a pre-verified pool). | **Medium / timing-bound** — the surface is copyable in 2-3 quarters. The durable part is grounding *discipline* welded to the owned, special-district-capable Shadow Atlas architecture (a latent moat, not a present $0-per-lookup cost edge — that $0 is just the congressional public-data floor, and the sold segment runs a paid agentic pipeline) and the API-collapse timing; the real edge is owning the abandoned market during the 2026–2027 distress window first. |
| **Shadow Atlas (district layer)** | An owned 24-slot H3 architecture (slot 0 congressional ingested; the other 23 non-congressional slots un-ingested; the special districts, slots 11–23, are the part no competitor covers), R-tree spatial index. Congressional district resolution + congressional officials resolve at $0 per lookup, but that $0 is only the congressional public-data floor (free TIGER + congress-legislators, available to Cicero too — not a structural cost edge); state/local official enrichment runs a paid agentic pipeline that plausibly costs more per lookup than Cicero. Live structured coverage is congressional (house rep + two senators via congress-legislators); special-district slots un-ingested — the only architecture built to host the 39,555 special districts' boundaries, a latent moat realized on ingestion. | **Low** — Google Civic shut down and the civic-data API collapse leaves Cicero the effective monopolist (volume discounts + free trial credits notwithstanding); owning the architecture during that window, plus the multi-year cost of cloning it, is the durable bet. The moat is latent + timing-bound, not a present per-lookup cost win. |
...
3. **Local-government / special-district advocacy** — the reach wedge. School boards, water districts, transit authorities, fire districts, judicial circuits. The owned 24-slot Shadow Atlas schema is the only architecture built to host these districts' boundaries — 23 of its 24 slots are Commons-built but un-ingested; the special districts (slots 11–23) are the part no competitor covers — a latent moat realized on ingestion: special-district slots un-ingested today, live structured reach congressional, local officials via a paid agentic search pipeline. An add-to-Quorum at $75, not a switch.
...
| Local government | Sub-state + special-district targeting | Federal + state legislative only | An owned 24-slot schema (slot 0 congressional ingested; the other 23 un-ingested; the special districts, slots 11–23, are the part no competitor covers) built to host school, water, fire, transit, judicial district boundaries — latent, not live: slots un-ingested today, live structured reach congressional, local officials via a paid agentic search pipeline |
