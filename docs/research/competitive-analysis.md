...
Google Civic Info's shutdown forced every advocacy platform to either pay Cicero or scramble for alternatives. **Commons owns the district layer** — Shadow Atlas (94,166 districts, a 24-slot H3 architecture, R-tree spatial indexing, sub-50ms p95). Zero marginal cost. No third-party dependency. Live structured coverage today is congressional (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested — the 24-slot schema is the only one that can ever cover them at $0, a latent moat.
...
|---|---|---|---|
| **District matching** | Full street address → Cicero API (~$0.03/lookup) | Postal code → Shadow Atlas (94,166 districts, 24-slot H3 architecture, sub-50ms) | Zero marginal cost. No third-party dependency. Live structured officials coverage is congressional (house rep + two senators); state/local via agentic search; special-district slots un-ingested — the 24-slot schema is the only one that can ever reach them, a latent moat. |
...
| Shadow Atlas (district tree) | Built (architecture); reach mostly latent | 94,166 districts, a 24-slot H3 architecture per cell, H3-indexed. Live structured coverage congressional; state/local via agentic search; special-district slots un-ingested — the 24-slot schema is the only one that can ever reach them at $0, a latent moat. |
