...
|---|---|---|
| AI-native authoring-to-delivery (grounded, set-tailored) | **TRANSCEND (authoring); delivery partial** | Source-grounded generation shipped + config-gated-live (Gemini + Exa stratified search + Firecrawl + adversarial-source-first ranking; grounding DISABLED so citations draw only from a pre-verified pool). No incumbent fetches + credibility-ranks external sources; all ship AI for the org user, not the action. Delivery is live for email + local-office channels; congressional CWC delivery is gated/dark (see "Where Commons trails"). |
| Local + special-district reach (Shadow Atlas) | **TRANSCEND** | The 24-slot H3 architecture ($0 marginal) is the only one that *can* cover special districts; the ~6 live TIGER/Line types (congressional, state-leg, county, municipal, school, VTD) already exceed Cicero (federal+state, ~400 cities, 0 special districts), with special-district breadth (toward 39,555 / ~84K officials) ingesting state-by-state. The durable moat is the owned data layer + architecture, not full nationwide special-district coverage today. |
