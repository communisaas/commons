...
FiscalNote's April 8, 2026 PolicyNote API launch added VoterVoice-powered address-to-district matching as a programmatic primitive, with explicit marketing language: *"Unlike competitors who rely on third-party vendors for district matching and inherit their limitations, FiscalNote owns and maintains its district data infrastructure."* This is market validation of the Shadow Atlas thesis — stated by an incumbent under going-concern pressure. VoterVoice covers federal/state/basic local; the Shadow Atlas differentiator vs every incumbent (FiscalNote owns its district data too) is not ownership — it is a $0-per-lookup cost structure (vs Cicero's $0.03–0.04/lookup) plus a 24-slot H3 architecture that can host special-district boundaries (water, fire, transit, etc.) Cicero structurally can't — a latent moat. The $0 applies to resolution of ingested boundaries (congressional today, house rep + two senators via congress-legislators) plus congressional officials; state and local officials resolve via a paid agentic search pipeline; special-district slots are un-ingested.
...
**Cicero (Melissa Data) is now the effective monopolist** for address→district at $0.03–0.04/lookup. A medium campaign (50K supporters) = $1,500–$2,000/campaign in Cicero fees; 10 campaigns/year = $15K–$20K/year just for district resolution. No commercial vendor matches Shadow Atlas's cost structure ($0 per lookup vs Cicero's $0.03–0.04) or its 24-slot architecture that can host special-district boundaries Cicero can't. The $0 is scoped to resolution of ingested boundaries (congressional today) plus congressional officials; state and local officials resolve via a paid agentic search pipeline, and special-district breadth is latent (un-ingested).
...
**Shadow Atlas position:** a 24-slot H3 architecture whose differentiator vs Cicero is not ownership (Cicero owns its data too) but a $0-per-lookup cost structure (vs Cicero's $0.03–0.04/lookup) plus the only architecture that can host special-district boundaries Cicero structurally can't (latent today). The $0 applies to resolution of ingested boundaries (congressional today, house rep + two senators via congress-legislators) plus congressional officials; state and local officials resolve via a paid agentic search pipeline; special-district slots un-ingested. FiscalNote's April 2026 PolicyNote API launch explicitly markets district matching as a moat — *"Unlike competitors who rely on third-party vendors..."* — market validation under duress.
...
2. **$0-per-lookup + special-district-capable reach (Shadow Atlas).** A 24-slot H3 architecture whose district resolution is $0 marginal — a $0-per-lookup cost structure vs Cicero's $0.03–0.04/lookup, not a data-ownership claim (Cicero owns its data too) — and **the only architecture that *can* host special-district boundaries at all**. That breadth is mostly **latent** today: the $0 applies to resolution of ingested boundaries — live structured coverage is **congressional** (the `cd` boundary bundles + officials the client actually consumes) — plus congressional officials; state and local officials resolve via **agentic search** (a paid LLM/search step, not the $0 static layer), not structured boundary→official data; and the special-district slots (water/fire/transit/school/library/hospital/judicial — toward 39,555 districts / ~84K officials) are **built but not yet ingested (0 today)**. The durable moat is the $0-per-lookup cost structure plus the only architecture that can *ever* host the special-district boundaries that Quorum Local (cities ≥3K pop, 0 special districts), Cicero (~400 cities, the $0.03–0.04/lookup monopoly), and VoterVoice (>250K population only) structurally cannot reach. The civic-data API collapse strengthens this reach moat; it does not make Commons a legislative-intelligence competitor.
...
| Local + special-district reach (Shadow Atlas) | **TRANSCEND (architecture); live ≈ congressional** | The 24-slot H3 schema resolves ingested boundaries at $0 marginal — a $0-per-lookup cost structure vs Cicero's $0.03–0.04/lookup, not a data-ownership claim (Cicero owns its data too) — and is the only one that *can* host special-district boundaries — a durable but **latent** moat. The $0 applies to resolution of ingested boundaries plus congressional officials; live structured coverage today is congressional (boundary + officials); state/local officials resolve via a paid agentic search pipeline; the special-district slots are built but **not yet ingested (0)**. Cicero (federal+state, ~400 cities, 0 special districts) structurally can't represent them. The moat is the $0-per-lookup cost structure + special-district-capable architecture + the API-collapse tailwind, not present-day breadth. |
...
Google Civic Info's shutdown forced every advocacy platform to either pay Cicero or scramble for alternatives. **Commons resolves the district layer at $0 per lookup** — Shadow Atlas (94,166 districts, a 24-slot H3 architecture, R-tree spatial indexing, sub-50ms p95). The differentiator vs Cicero is not ownership (Cicero owns its data too) but a $0-per-lookup cost structure (vs Cicero's $0.03–0.04/lookup) plus a 24-slot architecture that can host special-district boundaries Cicero can't. The $0 applies to resolution of ingested boundaries plus congressional officials; live structured coverage today is congressional (house rep + two senators via congress-legislators); state/local officials resolve via the paid agentic pipeline; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat.
...
**Commons's architecture addresses 24 boundary types per cell.** Shadow Atlas has ingested TIGER/Line congressional, state legislative, county, municipal, school district, and voting tabulation district *boundary geometry* — but live structured officials coverage is congressional only (house rep + two senators via congress-legislators); state/local via agentic search; special-district slots un-ingested. The architecture (H3-indexed, 24 slots per cell) is designed to ingest special-district boundaries as they become available — state by state, LAFCO by LAFCO. Every boundary type ingested then becomes targetable. No other platform's architecture can accommodate this; the 24-slot schema is the only one that can ever host special-district boundaries, a latent moat — a $0-per-lookup cost structure (vs Cicero's $0.03–0.04/lookup) plus special-district-capable architecture, not a data-ownership claim.

**The market opportunity:** A platform that lets a transit advocacy group, a school parent coalition, or a water district accountability org author verified campaigns to their specific local officials — free to author, $10 Starter to deliver — would have zero competition. Not underpriced competition. Zero. (Today this is the latent opportunity the architecture is built for, not present-day reach: the special-district slots are un-ingested, and local officials resolve via agentic search.)
...
Commons fills this vacuum structurally. Verification is orthogonal to ideology. The protocol doesn't check politics — it checks proof. A $10 Starter entry (with author-free onboarding) that includes email campaigns, verified advocacy, full uncapped API access, and district targeting — congressional live, state/local via agentic search, on a 24-slot schema that is the only one that can ever host special-district boundaries — is immediately the best tool a conservative advocacy org has ever had. It's also the best tool many progressive orgs have ever had — the ones too small for AN's $15/mo or excluded from EveryAction's ecosystem for other reasons.
...
|---|---|---|---|
| **District matching** | Full street address → Cicero API (~$0.03/lookup) | Postal code → Shadow Atlas (94,166 districts, 24-slot H3 architecture, sub-50ms) | Zero marginal cost per lookup vs Cicero's $0.03–0.04/lookup — a cost-structure win, not a data-ownership claim (Cicero owns its data too); no third-party dependency. The $0 applies to resolution of ingested boundaries plus congressional officials; state/local officials resolve via the paid agentic pipeline; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat. |
| **Official lookup** | Static database of ~500K officials (Cicero, Quorum, VoterVoice) | Congressional officials (house rep + two senators) from congress-legislators; state/local via a 3-phase agentic enrichment pipeline | Live structured officials coverage is congressional only; state/local officials via a paid agentic search pipeline. Special-district offices (water, transit, school board) un-ingested — the architecture can host their boundaries, slots empty today. |
...
| **List management / CRM** | Email-keyed records, tags, self-reported geography, engagement scores | Email-keyed records, tags, identity commitment binding, engagement tiers (on-chain), district membership across the 24-slot schema | Tiers are non-fakeable. District membership is cryptographically proven, not self-reported. Live structured coverage congressional; state/local officials via a paid agentic search pipeline; special-district slots un-ingested — latent capacity to host their boundaries. |
...
| **Advocate mapping** | District overlay on map (Muster, VoterVoice, Quorum). Typically federal + state legislative only. | Shadow Atlas: a 24-slot H3 architecture per cell. H3-indexed. 94,166 districts. | The 24-slot schema is the only one that can ever host water, fire, school, transit, judicial district boundaries — un-ingested today; live structured coverage congressional, state/local officials via a paid agentic search pipeline. Latent moat. |
...
| District proof | None | Merkle proof against the hierarchical district tree (24-slot schema can host special-district boundaries; live structured coverage congressional, special-district slots un-ingested) |
...
| **Letter campaigns** | Congress + state legislatures. District from self-reported ZIP. No way to verify the sender lives there. | Live structured reach is congressional (house rep + two senators via congress-legislators); state/local officials via a paid agentic search pipeline; the 24-slot schema can host special-district boundaries (fire, water, transit, school, judicial), un-ingested today. District from postal→district resolution + mDL. Letter carries a ZK proof. The letter is a verified civic instrument, not a form submission. |
| **List management** | Tags, CSV import/export, self-reported geography. An org's list is a collection of unverified claims. | Tags, CSV import/export, engagement tiers (non-fakeable, on-chain), district membership across the 24-slot schema (live structured coverage congressional; state/local officials via a paid agentic search pipeline; the schema can host special-district boundaries, un-ingested). An org's list is a set of cryptographically attested relationships. |
...
| List management / CRM | Upcoming | CSV import/export, tags, segments. Tier-aware. District membership across the 24-slot schema (live structured coverage congressional; state/local officials via a paid agentic search pipeline; the schema can host special-district boundaries, un-ingested). |
...
| Shadow Atlas (district tree) | Built (architecture); reach mostly latent | 94,166 districts, a 24-slot H3 architecture per cell, H3-indexed. Live structured coverage congressional; state/local officials via a paid agentic search pipeline; special-district slots un-ingested — the 24-slot schema is the only one that can ever host their boundaries, a latent moat. |
...
|---|---|---|
| **AI-native authoring-to-delivery spine** | End-to-end intent → ground → author → resolve-targets → deliver → report. Source-grounded composition tailored to the resolved decision-maker set (Exa + Firecrawl + adversarial-source-first ranking; grounding disabled so citations draw from a pre-verified pool). | **Medium / timing-bound** — the surface is copyable in 2-3 quarters. The durable part is grounding *discipline* welded to the Shadow Atlas $0-per-lookup cost structure + special-district-capable architecture; the real edge is owning the abandoned market during the 2026–2027 distress window first. |
| **Shadow Atlas (district layer)** | 94,166 districts, a 24-slot H3 architecture (21 slots with zero competitor coverage), R-tree spatial index. District resolution + congressional officials resolve at $0 per lookup (a cost-structure win vs Cicero's $0.03–0.04/lookup — not a data-ownership claim, since Cicero owns its data too); state/local official enrichment runs the paid agentic pipeline. Live structured coverage is congressional (house rep + two senators via congress-legislators); state/local via the paid agentic pipeline; special-district slots un-ingested — the only schema that can ever host the 39,555 special districts' boundaries, a latent moat. | **Low** — Cicero charges $0.03–0.04/lookup; Google Civic shut down; the civic-data API collapse strengthens this. Building a competing district layer at this scope is a multi-year effort. The durable moat. |
...
| Local government | Sub-state + special-district targeting | Federal + state legislative only | A 24-slot schema (21 slots with zero competitor coverage) that can host school, water, fire, transit, judicial district boundaries — latent, not live: slots un-ingested today, live structured reach congressional, local officials via a paid agentic search pipeline |
