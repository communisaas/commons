...
- A campaign from Action Network with N signatures, no identity verification, district from self-reported ZIP through Cicero (~$0.03/lookup), authorship indistinguishable from form mail.
...
6. **Long-term-survivable accountability artifacts.** Requires on-chain Merkle anchoring. AN's automation ladders aren't exportable; Quorum's reports live in vendor silos; nothing in the landscape produces audit-survivable receipts.
...
| --- | --- | --- | --- |
| C-coordination-integrity | 4 / 4 (**realized**) | 2 / 2 (**realized**) | **REALIZED across both sweeps** — reputation cron has real data via userId thread (NEW-E-1). **v3 GAP**: NEW-E-2 atlasVersion server-side thread is honest, but no current client populates the form field (`/c/[slug]`, `/embed/campaign/[slug]` /+page.svelte never write `atlasVersion` to formData) — packet driftCount surfaces null in practice until shadow-atlas-client→submission-form integration is built. See FIX-V2 (deferred). |
...
- **AN/RFP partnership outcome** — OSDI sync (T1-3), OSDI namespace (T9-4), state-bill ingestion (T6-6)
...
- **International extensibility.** Per-country district trees keyed on country code. UK/CA/AU postal resolvers staged (currently stubbed at the `LIVE_RESOLVER_COUNTRIES = ['US']` gate). The protocol structure supports any country whose boundary data is ingestible.

**Gap to close:** Local-government officeholder data (only federal `congress-legislators` is ingested; state and local rely on agentic Exa search). CA/GB/AU rep-lookup stubs. Mainnet `DistrictRegistry` deployment.
...
**Gap to close:** Coalition aggregation API endpoint (currently 501). Cross-border campaign data model (single `targetCountry` field today). Cross-org reputation aggregation query.
...
**Gap to close:** Auto-debate-spawn from campaign threshold (HTTP 501 today). On-chain stake verification in `createArgument` (currently caps at $1 placeholder). TEE-attested AI panel execution. Mainnet DebateMarket deployment.
...
**What transcends:**
- **Long-term-survivable artifacts.** Receipts anchor to Merkle roots on Scroll. A scorecard built today is auditable in 5 years even if the platform goes away. No app-layer competitor produces this — AN reports live in AN's database; Quorum reports live in Quorum's database; Bonterra reports live across seven Apax codebases. Commons receipts can survive the platform.
...
**Gap to close:** Coordination integrity score history (current scores work; no historical trend stored or charted). Identical-content flag with threshold (ALD prose warning is partial). Engagement tier histogram rendered in UI (computed but not displayed — see structural gap in [[tier-system-structural-gaps]]).
...
| 3. Composability | Protocol identity, verification packet, `verifyMdocDeviceAuth` shared | Coalition aggregation API (501 today); cross-border campaign schema (single targetCountry today); cross-org reputation query |
...
| 5. Quality signaling | DebateMarket on Sepolia, AI panel + EIP-712, stancePositions, position-privacy ZK | Auto-debate-spawn from campaign threshold (501); on-chain stake verification (capped at $1 placeholder); TEE-attested AI execution |
...
| 7. Coordination integrity | GDS, ALD, temporal entropy, burst velocity, CAI computed and rendered | Score history (no time-series); engagement tier histogram rendered; identical-content threshold flag |
...
The substrate carries the transcendent claim. Most of what makes Commons categorically different is built. The seven items to close map to **agentic execution** (delegation executor + per-supporter alerts), **coalition composition** (aggregation API + cross-border + mainnet contracts), **quality maturation** (auto-debate-spawn + on-chain stake verification + TEE), and **accountability anchoring** (receipt Merkle pipeline).
...
The substrate to make this real is largely shipped. The path forward is not "build more competitive features." It is closing the seven Phase 3 surfaces (delegation executor, per-supporter alerts, cross-border, mainnet, TEE, receipt anchoring, auto-debate-spawn) and shipping the reinstantiation of every existing advocacy modality through the capability composition.

## What this means for positioning

When asked "what does Commons do that AN/Quorum/EveryAction don't?" — the answer is not "we verify." The answer is the capability composition above, of which verification is one cluster among nine.
