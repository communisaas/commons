...
2. **An owned district layer built for 24 boundary types.** Requires *owning* the district layer (app-layer competitors structurally cannot). Quorum Local covers cities ≥3K population + ~75K official profiles; Cicero ~400 cities; FiscalNote Curate 16K entities; none of them cover the 39,555 special districts (water/fire/transit/library/hospital/judicial/etc.) governing 84K elected officials. Commons owns the 24-slot architecture for it — congressional ingested today, the special-district slots built but un-ingested (latent capacity, not present coverage).
...
8. **Owning the district-resolution layer post-API-collapse.** Requires owning the layer rather than renting Cicero (the effective monopolist at $0.03–$0.04/lookup post-Google-Civic-shutdown). The $0 floor today is only the congressional public data (free to all); the structural value is owning the layer as the API ecosystem dies — realized as the other slots are ingested.
...
| --- | --- | --- | --- |
| C-coordination-integrity | 4 / 4 (**realized**) | 2 / 2 (**realized**) | **REALIZED across both sweeps** — the userId thread lets each registered verified action update reputation and immutable engagement attribution transactionally (NEW-E-1). **v3 data-honesty update:** NEW-E-2 atlasVersion server-side thread is honest, and both `/c/[slug]` verified-address submissions and `/embed/campaign/[slug]` district-evidence submissions now carry H3 cell + atlas version after successful resolver output. Packet driftCount is available when action rows carry atlas evidence; postal-only/skipped district-evidence actions remain outside that broad claim. |
...
**Shipped:** Shadow Atlas with a 24-boundary-type H3 res-7 cell *architecture* (US_SLOT_NAMES) — only the congressional layer (slot 0) is ingested today; slots 1–23 (state/county/municipal/school/water/fire/transit/etc.) are empty schema capacity. R-tree p95 <50ms, R2-chunked distribution. The congressional read path resolves at $0 marginal off free public data (TIGER + congress-legislators) — the congressional public-data floor, available to Cicero too, not a structural per-lookup cost advantage; the sold state/local/special-district segment resolves officials via a paid agentic pipeline (Gemini + Exa + Firecrawl). Country-code-keyed `DistrictRegistry` contract. Atlas Worker pipeline live on Cloudflare R2.

**What transcends:**
- **Local government void.** The 39,555 special districts governing 84K officials are not covered by any platform today — including Commons (slots 1–23 are un-ingested; state/local officials reach the user only via paid agentic Exa search, with no structured backing). The durable asset is architecture-vs-architecture: Commons owns the only schema *built* to host these districts — Quorum Local (~75K official profiles, cities ≥3K population, 0 special districts), Cicero (~400 cities), and VoterVoice (>250K population only) are not built for them, and list-first petition tools do not own this terrain. Commons's 24 boundary types (water/fire/transit/hospital/library/park/judicial/township/precinct/tribal) are built-out slots, empty today — latent capacity, not present coverage.
- **Independence from the dying API layer.** Google Civic shut April 2025; ProPublica Congress API shut July 2024; OpenSecrets API shut April 2025; Congress.gov API outage August 2025 with no restoration timeline. Cicero is the effective monopolist at $0.03–$0.04/lookup. Commons owns the layer — a latent, timing-bound moat: today only the congressional slot is ingested (so present coverage is congressional, not 24x anyone), and the $0 marginal is just the congressional public-data floor (free TIGER + congress-legislators), not a structural cost edge — the sold state/local segment runs a paid agentic pipeline that plausibly costs more per lookup than Cicero. Owning the layer matters *if/when* the other 23 slots are ingested; that is the path to $0 special-district resolution, and the API collapse is the timing window that makes owning the layer the durable bet.
...
**Shipped:** Shadow Atlas owned in-house — a 24-boundary-type architecture of which only the congressional layer (slot 0) is ingested today; slots 1–23 are empty schema capacity. The congressional read path is $0 marginal off free public data (TIGER + congress-legislators) — the congressional public-data floor, not a per-lookup cost advantage for the sold state/local segment, which runs a paid agentic pipeline. Ownership of the architecture is the durable/latent asset (value realized on ingestion), not a present cost edge. PII-free architecture (encrypted PII via org key; user PII fully eliminated 2026-04-10 — `_pii.ts`, `user-pii-encryption.ts`, `PII_ENCRYPTION_KEY`, `EMAIL_LOOKUP_KEY` all removed). Voter-protocol/* domain strings frozen post-launch.

**What transcends:**
- **Independence from the dying civic-data API layer.** Google Civic shut April 2025. ProPublica Congress API shut July 2024. OpenSecrets API shut April 2025. Congress.gov API outage August 2025 with no restoration timeline. Cicero is the effective monopolist at $0.03–$0.04/lookup. Commons's congressional path sits outside the decay (it reads public TIGER + congress-legislators, not the dead APIs); the state/local segment is not yet served by an owned, ingested layer — it depends on agentic Exa/Firecrawl web search, its own external dependency. Full independence is the realized-on-ingestion outcome of owning the layer (the durable, timing-bound bet), not a present state across all jurisdictions.
...
| 2. Reach | 24-slot Shadow Atlas architecture (congressional ingested), country-code registries | Special-district + local/state officeholder data (only agentic Exa today); CA/GB/AU rep-lookup; mainnet `DistrictRegistry` |
...
When asked "what's the moat?" — the verification substrate (Cluster 1, the ZK/mDL spine) is the genuinely-present, defensible moat: it cannot be retrofitted to app-layer competitors. The reach substrate (Clusters 2 + 9) is a *latent*, timing-bound moat, not a present cost or coverage edge: Commons owns the only architecture built to host the local-government layer, but only the congressional slot is ingested today, so the $0/coverage advantage is realized *if/when* the other slots are ingested — the path to $0 special-district resolution welded to the API-collapse timing window. FiscalNote's April 2026 District Matching API launch under going-concern pressure is the market validation: an incumbent in distress argued that owning district infrastructure is a moat. They're right — and that validates the architectural/latent bet. Commons owns the substrate; they don't.
