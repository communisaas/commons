...
- **F-153 ✅ CURED:** segment-filter `engagementTier` case read a non-existent supporter field. Server-side now evaluates it from indexed `campaignActions` context as the supporter max action tier; `campaignParticipation` and `actionDistrict` use the same action-context join. Human-readable state/local labels remain deferred to civic-geography denormalization.
