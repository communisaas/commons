...
The Studio capability map and its canvas cluster were deleted. Org members no longer see a workspace-posture readout, capability coverage grid, gate register, launch-pressure rows, loop/lattice matrices, operator queue, or any of the readiness instruments those surfaces rendered. Internal readiness and posture state (armed/bounded/held, gate evidence, claim boundaries) is no longer rendered to orgs at all.

What an org member sees instead is plain org-words: the Results evidence page, and the People, Power, and Studio spaces. Each action that works today within a real limit shows one plain limit sentence through `BoundedNotice` (`src/lib/components/org/BoundedNotice.svelte`), keyed off the server boundary codes in `src/lib/data/org-limit-sentences.ts`. A kept artifact (draft, workflow) gets a quiet preserved-artifact reassurance, and the operator runtime detail stays collapsed behind an administrator affordance. The readiness builders still exist as server-side logic that gates behavior, but their state machine vocabulary is no longer surfaced as an org-facing instrument.
...
- Delivery metrics (`campaignReadModels` delivery counters, surfaced through `src/lib/server/campaign-read-model.ts`): ✅ sent/delivered/opened/bounced plus observed verify-link clicks from `campaignDeliveries.responses` / `accountabilityReceipts.responses`
...
**LLM rate limits** ✅
`src/lib/server/llm-cost-protection.ts`. 3-tier (guest/auth/verified). Per-operation + daily global. 3/10/15 per day daily-global. `message-generation` 0/3/5 per hour. `decision-makers` 0/2/3 per hour. `subject-line` 0/5/5 per hour. Paid agent routes require a session before rate limiting or provider work; guest model work remains disabled until globally coordinated admission exists. Fail-closed unknown ops. Authenticated quotas remain per-isolate defense in depth.
...
**Email analytics** ✅
Per-blast totals on `emailBlasts`. `emailEvents` per-recipient. `/org/[slug]/emails/[blastId]/` reads `email.getBlast` for the blast row and `email.listReceiptsForBlast` for a bounded receipt page (50 rows) it reduces to sent/failed counts.
...
**Atlas-version drift surface** ✅ — H6 `atlasDrift` + `atlasDriftLabel` in TierDisplay, unified across the email footer and /v/[hash].
...
**Analytics expanded** ✅
`src/lib/server/campaign-read-model.ts` powers delivery metrics, timeline buckets, geographic spread, verification counts. `FEATURES.ANALYTICS_EXPANDED=true`. Components: DeliveryMetrics, VerificationTimeline, GeographicSpread.
